 ...
 * Page class
 */
class WebsiteMenuEdit extends WebsiteMenu
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WebsiteMenuEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("MENU_ID") ?? Key(0) ?? Route(2)) !== null) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // javascript_id
            $this->javascript_id->setDbValueDef($rsnew, $this->javascript_id->CurrentValue, null, $this->javascript_id->ReadOnly);

            // file_name
            $this->file_name->setDbValueDef($rsnew, $this->file_name->CurrentValue, null, $this->file_name->ReadOnly);

            // menu_name
            $this->menu_name->setDbValueDef($rsnew, $this->menu_name->CurrentValue, null, $this->menu_name->ReadOnly);

            // isactive
            $this->isactive->setDbValueDef($rsnew, $this->isactive->CurrentValue, null, $this->isactive->ReadOnly);

            // menu_type
            $this->menu_type->setDbValueDef($rsnew, $this->menu_type->CurrentValue, null, $this->menu_type->ReadOnly);

            // header_name
            $this->header_name->setDbValueDef($rsnew, $this->header_name->CurrentValue, null, $this->header_name->ReadOnly);

            // isslide
            $this->isslide->setDbValueDef($rsnew, $this->isslide->CurrentValue, null, $this->isslide->ReadOnly);

            // timeslide
            $this->timeslide->setDbValueDef($rsnew, $this->timeslide->CurrentValue, null, $this->timeslide->ReadOnly);
