 ...
 * Page class
 */
class VisitTypeAdd extends VisitType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VisitTypeAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("ISNEW") ?? Route("ISNEW")) !== null) {
                $this->ISNEW->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("VisitTypeList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "VisitTypeList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "VisitTypeView") {
    ...
    {
        global $Language, $Security;
        if ($this->ISNEW->CurrentValue != "") { // Check field with unique index
            $filter = "([ISNEW] = '" . AdjustSql($this->ISNEW->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->ISNEW->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->ISNEW->CurrentValue, $idxErrMsg);
        ...
        // ISNEW
        $this->ISNEW->setDbValueDef($rsnew, $this->ISNEW->CurrentValue, "", false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['ISNEW']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VisitTypeList"), "", $this->TableVar, true);
