 ...
 * Page class
 */
class UserAccessList extends UserAccess
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'USER_ACCESS';

    // Page object name
    public $PageObjName = "UserAccessList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fUSER_ACCESSlist";
    ...
    public function __construct()
    {
        ...
        // Table object (USER_ACCESS)
        if (!isset($GLOBALS["USER_ACCESS"]) || get_class($GLOBALS["USER_ACCESS"]) == PROJECT_NAMESPACE . "USER_ACCESS") {
            $GLOBALS["USER_ACCESS"] = &$this;
        }
        ...
        $this->AddUrl = "UserAccessAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "UserAccessDelete";
        $this->MultiUpdateUrl = "UserAccessUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'USER_ACCESS');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fUSER_ACCESSlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("USER_ACCESS"));
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        ...
        if (is_array($ar)) {
            ...
            $key .= @$ar['USE_ORG_UNIT_CODE'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['GROUP_ID'];
        }
        return $key;
    }
    ...
    public function run()
    {
        ...
        $this->USE_ORG_UNIT_CODE->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->USE_ORG_UNIT_CODE->AdvancedSearch->toJson(), ","); // Field USE_ORG_UNIT_CODE
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fUSER_ACCESSlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field USE_ORG_UNIT_CODE
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->SearchValue = @$filter["x_USE_ORG_UNIT_CODE"];
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->SearchOperator = @$filter["z_USE_ORG_UNIT_CODE"];
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->SearchCondition = @$filter["v_USE_ORG_UNIT_CODE"];
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->SearchValue2 = @$filter["y_USE_ORG_UNIT_CODE"];
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->SearchOperator2 = @$filter["w_USE_ORG_UNIT_CODE"];
        $this->USE_ORG_UNIT_CODE->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->USE_ORG_UNIT_CODE, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->USE_ORG_UNIT_CODE); // USE_ORG_UNIT_CODE
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->USE_ORG_UNIT_CODE->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->ORG_UNIT_CODE->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->_USERNAME->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->USE_ORG_UNIT_CODE->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->GROUP_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fUSER_ACCESSlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fUSER_ACCESSlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fUSER_ACCESSlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fUSER_ACCESSlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
