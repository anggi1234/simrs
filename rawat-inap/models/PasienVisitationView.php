...
class PasienVisitationView extends PasienVisitation
{
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("VAkomodasiKamarList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        // "detail_V_REG_AKOMODASI"
        $item = &$option->add("detail_V_REG_AKOMODASI");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("V_REG_AKOMODASI", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("VRegAkomodasiList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("VRegAkomodasiGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'PASIEN_VISITATION')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=V_REG_AKOMODASI"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "V_REG_AKOMODASI";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'PASIEN_VISITATION')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=V_REG_AKOMODASI"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "V_REG_AKOMODASI";
        }
        if ($links != "") {
            $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-toggle=\"dropdown\"></button>";
            $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        $body = "<div class=\"btn-group btn-group-sm ew-btn-group\">" . $body . "</div>";
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'V_REG_AKOMODASI');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "V_REG_AKOMODASI";
        }
        if ($this->ShowMultipleDetails) {
            $item->Visible = false;
        }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("V_REG_AKOMODASI", $detailTblVar)) {
                $detailPageObj = Container("VRegAkomodasiGrid");
                if ($detailPageObj->DetailView) {
                    $detailPageObj->CurrentMode = "view";
