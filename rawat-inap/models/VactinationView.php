 ...
 * Page class
 */
class VactinationView extends Vactination
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VactinationView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("VACTINATION_ID") ?? Route("VACTINATION_ID")) !== null) {
            $this->RecKey["VACTINATION_ID"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("ORG_UNIT_CODE") ?? Route("ORG_UNIT_CODE")) !== null) {
                ...
                $returnUrl = "VactinationList"; // Return to list
            }
            if (($keyValue = Get("VACTINATION_ID") ?? Route("VACTINATION_ID")) !== null) {
                $this->VACTINATION_ID->setQueryStringValue($keyValue);
                $this->RecKey["VACTINATION_ID"] = $this->VACTINATION_ID->QueryStringValue;
            } elseif (Post("VACTINATION_ID") !== null) {
                $this->VACTINATION_ID->setFormValue(Post("VACTINATION_ID"));
                $this->RecKey["VACTINATION_ID"] = $this->VACTINATION_ID->FormValue;
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "VactinationList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "VactinationList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
