...
class PasienVisitation extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // NO_REGISTRATION
        $this->NO_REGISTRATION = new DbField('PASIEN_VISITATION', 'PASIEN_VISITATION', 'x_NO_REGISTRATION', 'NO_REGISTRATION', '[NO_REGISTRATION]', '[NO_REGISTRATION]', 200, 50, -1, false, '[NO_REGISTRATION]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // IDXDAFTAR
        $this->IDXDAFTAR = new DbField('PASIEN_VISITATION', 'PASIEN_VISITATION', 'x_IDXDAFTAR', 'IDXDAFTAR', '[IDXDAFTAR]', 'CAST([IDXDAFTAR] AS NVARCHAR)', 3, 4, -1, false, '[IDXDAFTAR]', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "V_REG_AKOMODASI") {
            $detailUrl = Container("V_REG_AKOMODASI")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue);
    ...
    {
        // Cascade Update detail table 'V_REG_AKOMODASI'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("V_REG_AKOMODASI")->loadRs("dbo.TREATMENT_AKOMODASI.NO_REGISTRATION = " . QuotedValue($rsold['NO_REGISTRATION'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.VISIT_ID = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.THENAME = " . QuotedValue($rsold['DIANTAR_OLEH'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.TRANS_ID = " . QuotedValue($rsold['TRANS_ID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("V_REG_AKOMODASI")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("V_REG_AKOMODASI")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("V_REG_AKOMODASI")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'V_REG_AKOMODASI'
        $dtlrows = Container("V_REG_AKOMODASI")->loadRs("dbo.TREATMENT_AKOMODASI.NO_REGISTRATION = " . QuotedValue($rs['NO_REGISTRATION'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.VISIT_ID = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.THENAME = " . QuotedValue($rs['DIANTAR_OLEH'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.TRANS_ID = " . QuotedValue($rs['TRANS_ID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("V_REG_AKOMODASI")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("V_REG_AKOMODASI")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("V_REG_AKOMODASI")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->IN_DATE->EditValue = $this->IN_DATE->CurrentValue;
        $this->IN_DATE->EditValue = FormatDateTime($this->IN_DATE->EditValue, 11);
        $this->IN_DATE->ViewCustomAttributes = "";
