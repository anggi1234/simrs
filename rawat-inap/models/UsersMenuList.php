 ...
 * Page class
 */
class UsersMenuList extends UsersMenu
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UsersMenuList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fUSERS_MENUlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "UsersMenuAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "UsersMenuDelete";
        $this->MultiUpdateUrl = "UsersMenuUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fUSERS_MENUlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->U->AdvancedSearch->toJson(), ","); // Field U
        ...
        $filterList = Concat($filterList, $this->C_TIME->AdvancedSearch->toJson(), ","); // Field C_TIME
        $filterList = Concat($filterList, $this->U_TIME->AdvancedSearch->toJson(), ","); // Field U_TIME
        $filterList = Concat($filterList, $this->D_TIME->AdvancedSearch->toJson(), ","); // Field D_TIME
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fUSERS_MENUlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field C_TIME
        $this->C_TIME->AdvancedSearch->SearchValue = @$filter["x_C_TIME"];
        $this->C_TIME->AdvancedSearch->SearchOperator = @$filter["z_C_TIME"];
        $this->C_TIME->AdvancedSearch->SearchCondition = @$filter["v_C_TIME"];
        $this->C_TIME->AdvancedSearch->SearchValue2 = @$filter["y_C_TIME"];
        $this->C_TIME->AdvancedSearch->SearchOperator2 = @$filter["w_C_TIME"];
        $this->C_TIME->AdvancedSearch->save();

        // Field U_TIME
        $this->U_TIME->AdvancedSearch->SearchValue = @$filter["x_U_TIME"];
        $this->U_TIME->AdvancedSearch->SearchOperator = @$filter["z_U_TIME"];
        $this->U_TIME->AdvancedSearch->SearchCondition = @$filter["v_U_TIME"];
        $this->U_TIME->AdvancedSearch->SearchValue2 = @$filter["y_U_TIME"];
        $this->U_TIME->AdvancedSearch->SearchOperator2 = @$filter["w_U_TIME"];
        $this->U_TIME->AdvancedSearch->save();

        // Field D_TIME
        $this->D_TIME->AdvancedSearch->SearchValue = @$filter["x_D_TIME"];
        $this->D_TIME->AdvancedSearch->SearchOperator = @$filter["z_D_TIME"];
        $this->D_TIME->AdvancedSearch->SearchCondition = @$filter["v_D_TIME"];
        $this->D_TIME->AdvancedSearch->SearchValue2 = @$filter["y_D_TIME"];
        $this->D_TIME->AdvancedSearch->SearchOperator2 = @$filter["w_D_TIME"];
        $this->D_TIME->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->MENU_ID, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->C, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->R, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->U, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->C_TIME); // C_TIME
            $this->updateSort($this->U_TIME); // U_TIME
            $this->updateSort($this->D_TIME); // D_TIME
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->ORG_UNIT_CODE->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->_USERNAME->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->MENU_ID->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->STYPE_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fUSERS_MENUlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fUSERS_MENUlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fUSERS_MENUlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fUSERS_MENUlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
