 ...
 * Page class
 */
class TreatmentBoundEdit extends TreatmentBound
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TreatmentBoundEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("REAGENT_ID") ?? Key(1) ?? Route(3)) !== null) {
                $this->REAGENT_ID->setQueryStringValue($keyValue);
                $this->REAGENT_ID->setOldValue($this->REAGENT_ID->QueryStringValue);
            } elseif (Post("REAGENT_ID") !== null) {
                $this->REAGENT_ID->setFormValue(Post("REAGENT_ID"));
                $this->REAGENT_ID->setOldValue($this->REAGENT_ID->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("REAGENT_ID") ?? Route("REAGENT_ID")) !== null) {
                    $this->REAGENT_ID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->REAGENT_ID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("TreatmentBoundList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TreatmentBoundList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'REAGENT_ID' first before field var 'x_REAGENT_ID'
        $val = $CurrentForm->hasValue("REAGENT_ID") ? $CurrentForm->getValue("REAGENT_ID") : $CurrentForm->getValue("x_REAGENT_ID");
        if (!$this->REAGENT_ID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->REAGENT_ID->Visible = false; // Disable update for API request
            } else {
                $this->REAGENT_ID->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_REAGENT_ID")) {
            $this->REAGENT_ID->setOldValue($CurrentForm->getValue("o_REAGENT_ID"));
        }
        ...
        // Check field name 'REAGENT_NAME' first before field var 'x_REAGENT_NAME'
        $val = $CurrentForm->hasValue("REAGENT_NAME") ? $CurrentForm->getValue("REAGENT_NAME") : $CurrentForm->getValue("x_REAGENT_NAME");
        if (!$this->REAGENT_NAME->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->REAGENT_NAME->Visible = false; // Disable update for API request
            } else {
                $this->REAGENT_NAME->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->REAGENT_ID->CurrentValue = $this->REAGENT_ID->FormValue;
        $this->CLINIC_ID->CurrentValue = $this->CLINIC_ID->FormValue;
        $this->TREAT_ID->CurrentValue = $this->TREAT_ID->FormValue;
        $this->REAGENT_NAME->CurrentValue = $this->REAGENT_NAME->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->REAGENT_ID->EditValue = HtmlEncode($this->REAGENT_ID->CurrentValue);
            ...
            $this->REAGENT_NAME->EditValue = HtmlEncode($this->REAGENT_NAME->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->REAGENT_ID->Required) {
            if (!$this->REAGENT_ID->IsDetailKey && EmptyValue($this->REAGENT_ID->FormValue)) {
                $this->REAGENT_ID->addErrorMessage(str_replace("%s", $this->REAGENT_ID->caption(), $this->REAGENT_ID->RequiredErrorMessage));
            }
        }
        ...
        if ($this->REAGENT_NAME->Required) {
            if (!$this->REAGENT_NAME->IsDetailKey && EmptyValue($this->REAGENT_NAME->FormValue)) {
                $this->REAGENT_NAME->addErrorMessage(str_replace("%s", $this->REAGENT_NAME->caption(), $this->REAGENT_NAME->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // REAGENT_ID
            $this->REAGENT_ID->setDbValueDef($rsnew, $this->REAGENT_ID->CurrentValue, "", $this->REAGENT_ID->ReadOnly);

            // CLINIC_ID
            $this->CLINIC_ID->setDbValueDef($rsnew, $this->CLINIC_ID->CurrentValue, null, $this->CLINIC_ID->ReadOnly);

            // TREAT_ID
            $this->TREAT_ID->setDbValueDef($rsnew, $this->TREAT_ID->CurrentValue, "", $this->TREAT_ID->ReadOnly);

            // REAGENT_NAME
            $this->REAGENT_NAME->setDbValueDef($rsnew, $this->REAGENT_NAME->CurrentValue, null, $this->REAGENT_NAME->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TreatmentBoundList"), "", $this->TableVar, true);
        $pageId = "edit";
