 ...
 * Page class
 */
class VactinationTypeEdit extends VactinationType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VactinationTypeEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("VACTINATION_TYPE") ?? Key(0) ?? Route(2)) !== null) {
                $this->VACTINATION_TYPE->setQueryStringValue($keyValue);
                $this->VACTINATION_TYPE->setOldValue($this->VACTINATION_TYPE->QueryStringValue);
            } elseif (Post("VACTINATION_TYPE") !== null) {
                $this->VACTINATION_TYPE->setFormValue(Post("VACTINATION_TYPE"));
                $this->VACTINATION_TYPE->setOldValue($this->VACTINATION_TYPE->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_VACTINATION_TYPE")) {
            $this->VACTINATION_TYPE->setOldValue($CurrentForm->getValue("o_VACTINATION_TYPE"));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // VACTINATION_TYPE
            $this->VACTINATION_TYPE->setDbValueDef($rsnew, $this->VACTINATION_TYPE->CurrentValue, 0, $this->VACTINATION_TYPE->ReadOnly);

            // DIAGNOSA_ID
            $this->DIAGNOSA_ID->setDbValueDef($rsnew, $this->DIAGNOSA_ID->CurrentValue, null, $this->DIAGNOSA_ID->ReadOnly);

            // VACTINATIONTYPE
            $this->VACTINATIONTYPE->setDbValueDef($rsnew, $this->VACTINATIONTYPE->CurrentValue, null, $this->VACTINATIONTYPE->ReadOnly);
