...
class PasienVisitationEdit extends PasienVisitation
{
    ...
    public function run()
    {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                ...
                break;
            case "update": // Update
                $returnUrl = 'PasienVisitationView/'.urlencode(CurrentPage()->IDXDAFTAR->CurrentValue).'?showdetail=TREATMENT_BILL';
    ...
    protected function validateForm()
    {
        ...
        if (in_array("V_REG_AKOMODASI", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        ...
        if (!$rsold) {
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    $detailPage = Container("VRegAkomodasiGrid");
                    if (in_array("V_REG_AKOMODASI", $detailTblVar) && $detailPage->DetailEdit) {
                        $Security->loadCurrentUserLevel($this->ProjectID . "V_REG_AKOMODASI"); // Load user level of detail table
                        $editRow = $detailPage->gridUpdate();
                        $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                    }
                }
