 ...
 * Page class
 */
class UserAccessEdit extends UserAccess
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserAccessEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("USE_ORG_UNIT_CODE") ?? Key(2) ?? Route(4)) !== null) {
                $this->USE_ORG_UNIT_CODE->setQueryStringValue($keyValue);
                $this->USE_ORG_UNIT_CODE->setOldValue($this->USE_ORG_UNIT_CODE->QueryStringValue);
            } elseif (Post("USE_ORG_UNIT_CODE") !== null) {
                $this->USE_ORG_UNIT_CODE->setFormValue(Post("USE_ORG_UNIT_CODE"));
                $this->USE_ORG_UNIT_CODE->setOldValue($this->USE_ORG_UNIT_CODE->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("GROUP_ID") ?? Key(3) ?? Route(5)) !== null) {
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("USE_ORG_UNIT_CODE") ?? Route("USE_ORG_UNIT_CODE")) !== null) {
                    $this->USE_ORG_UNIT_CODE->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->USE_ORG_UNIT_CODE->CurrentValue = null;
                }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("UserAccessList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "UserAccessList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'USE_ORG_UNIT_CODE' first before field var 'x_USE_ORG_UNIT_CODE'
        $val = $CurrentForm->hasValue("USE_ORG_UNIT_CODE") ? $CurrentForm->getValue("USE_ORG_UNIT_CODE") : $CurrentForm->getValue("x_USE_ORG_UNIT_CODE");
        if (!$this->USE_ORG_UNIT_CODE->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->USE_ORG_UNIT_CODE->Visible = false; // Disable update for API request
            } else {
                $this->USE_ORG_UNIT_CODE->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_USE_ORG_UNIT_CODE")) {
            $this->USE_ORG_UNIT_CODE->setOldValue($CurrentForm->getValue("o_USE_ORG_UNIT_CODE"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->USE_ORG_UNIT_CODE->CurrentValue = $this->USE_ORG_UNIT_CODE->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->USE_ORG_UNIT_CODE->EditValue = HtmlEncode($this->USE_ORG_UNIT_CODE->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->USE_ORG_UNIT_CODE->Required) {
            if (!$this->USE_ORG_UNIT_CODE->IsDetailKey && EmptyValue($this->USE_ORG_UNIT_CODE->FormValue)) {
                $this->USE_ORG_UNIT_CODE->addErrorMessage(str_replace("%s", $this->USE_ORG_UNIT_CODE->caption(), $this->USE_ORG_UNIT_CODE->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // USE_ORG_UNIT_CODE
            $this->USE_ORG_UNIT_CODE->setDbValueDef($rsnew, $this->USE_ORG_UNIT_CODE->CurrentValue, "", $this->USE_ORG_UNIT_CODE->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("UserAccessList"), "", $this->TableVar, true);
        $pageId = "edit";
