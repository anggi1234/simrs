 ...
 * Page class
 */
class VactinationTypeAdd extends VactinationType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VactinationTypeAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("VACTINATION_TYPE") ?? Route("VACTINATION_TYPE")) !== null) {
                $this->VACTINATION_TYPE->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("VactinationTypeList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "VactinationTypeList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "VactinationTypeView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->VACTINATIONTYPE->CurrentValue = null;
        $this->VACTINATIONTYPE->OldValue = $this->VACTINATIONTYPE->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'VACTINATIONTYPE' first before field var 'x_VACTINATIONTYPE'
        $val = $CurrentForm->hasValue("VACTINATIONTYPE") ? $CurrentForm->getValue("VACTINATIONTYPE") : $CurrentForm->getValue("x_VACTINATIONTYPE");
        if (!$this->VACTINATIONTYPE->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->VACTINATIONTYPE->Visible = false; // Disable update for API request
            } else {
                $this->VACTINATIONTYPE->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->VACTINATIONTYPE->CurrentValue = $this->VACTINATIONTYPE->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['VACTINATIONTYPE'] = $this->VACTINATIONTYPE->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->VACTINATIONTYPE->EditValue = HtmlEncode($this->VACTINATIONTYPE->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->VACTINATIONTYPE->Required) {
            if (!$this->VACTINATIONTYPE->IsDetailKey && EmptyValue($this->VACTINATIONTYPE->FormValue)) {
                $this->VACTINATIONTYPE->addErrorMessage(str_replace("%s", $this->VACTINATIONTYPE->caption(), $this->VACTINATIONTYPE->RequiredErrorMessage));
            }
        }
    ...
    {
        global $Language, $Security;
        if ($this->VACTINATION_TYPE->CurrentValue != "") { // Check field with unique index
            $filter = "([VACTINATION_TYPE] = " . AdjustSql($this->VACTINATION_TYPE->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->VACTINATION_TYPE->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->VACTINATION_TYPE->CurrentValue, $idxErrMsg);
        ...
        // VACTINATION_TYPE
        $this->VACTINATION_TYPE->setDbValueDef($rsnew, $this->VACTINATION_TYPE->CurrentValue, 0, false);

        // DIAGNOSA_ID
        $this->DIAGNOSA_ID->setDbValueDef($rsnew, $this->DIAGNOSA_ID->CurrentValue, null, false);

        // VACTINATIONTYPE
        $this->VACTINATIONTYPE->setDbValueDef($rsnew, $this->VACTINATIONTYPE->CurrentValue, null, false);

        // Call Row Inserting event
        $insertRow = $this->rowInserting($rsold, $rsnew);

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['VACTINATION_TYPE']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VactinationTypeList"), "", $this->TableVar, true);
