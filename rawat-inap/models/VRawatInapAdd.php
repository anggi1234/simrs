...
class VRawatInapAdd extends VRawatInap
{
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->modified_datesys->IsDetailKey) {
            ...
            $this->modified_datesys->CurrentValue = UnFormatDateTime($this->modified_datesys->CurrentValue, 11);
        }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("OBSTETRI", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // modified_datesys
        $this->modified_datesys->CurrentValue = CurrentDateTime();
        $this->modified_datesys->setDbValueDef($rsnew, $this->modified_datesys->CurrentValue, null);
        ...
        if ($addRow) {
            ...
            if (in_array("PASIEN_DIAGNOSA", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->VISIT_ID->setSessionValue($this->VISIT_ID->CurrentValue); // Set master key
                $detailPage->THENAME->setSessionValue($this->THENAME->CurrentValue); // Set master key
                $detailPage->NO_REGISTRATION->setSessionValue($this->NO_REGISTRATION->CurrentValue); // Set master key
                $detailPage->THEADDRESS->setSessionValue($this->THEADDRESS->CurrentValue); // Set master key
            ...
            if (in_array("OBSTETRI", $detailTblVar) && $detailPage->DetailAdd) {
                ...
                $detailPage->THENAME->setSessionValue($this->THENAME->CurrentValue); // Set master key
                $detailPage->THEADDRESS->setSessionValue($this->THEADDRESS->CurrentValue); // Set master key
                $detailPage->GENDER->setSessionValue($this->GENDER->CurrentValue); // Set master key
                $detailPage->ORG_UNIT_CODE->setSessionValue($this->ORG_UNIT_CODE->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "OBSTETRI"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->VISIT_ID->setSessionValue(""); // Clear master key if insert failed
                ...
                $detailPage->THENAME->setSessionValue(""); // Clear master key if insert failed
                $detailPage->THEADDRESS->setSessionValue(""); // Clear master key if insert failed
                $detailPage->GENDER->setSessionValue(""); // Clear master key if insert failed
                $detailPage->ORG_UNIT_CODE->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
