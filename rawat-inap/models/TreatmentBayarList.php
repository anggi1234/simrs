 ...
 * Page class
 */
class TreatmentBayarList extends TreatmentBayar
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TreatmentBayarList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fTREATMENT_BAYARlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "TreatmentBayarDelete";
        $this->MultiUpdateUrl = "TreatmentBayarUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fTREATMENT_BAYARlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fTREATMENT_BAYARlistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->SPPFEE, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SPPBILL, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SPPRJK, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SPPJMN, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SPPKASIR, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->PERUJUK, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->SPPFEE); // SPPFEE
            $this->updateSort($this->SPPBILL); // SPPBILL
            $this->updateSort($this->SPPRJK); // SPPRJK
            $this->updateSort($this->SPPJMN); // SPPJMN
            $this->updateSort($this->SPPKASIR); // SPPKASIR
            $this->updateSort($this->PERUJUK); // PERUJUK
            $this->updateSort($this->PERUJUKFEE); // PERUJUKFEE
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fTREATMENT_BAYARlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fTREATMENT_BAYARlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fTREATMENT_BAYARlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fTREATMENT_BAYARlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
