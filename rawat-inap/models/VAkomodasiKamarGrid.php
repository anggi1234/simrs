...
class VAkomodasiKamarGrid extends VAkomodasiKamar
{
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $masterFilter = $this->sqlMasterFilter_TREATMENT_BILL();
            $keyValue = $rsnew['BILL_ID'] ?? $rsold['BILL_ID'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@BILL_ID@", AdjustSql($keyValue), $masterFilter);
            } else {
            ...
            if ($validMasterRecord) {
                $rsmaster = Container("TREATMENT_BILL")->loadRs($masterFilter)->fetch();
                $validMasterRecord = $rsmaster !== false;
            }
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "TREATMENT_BILL", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        $masterFilter = str_replace("@BILL_ID@", AdjustSql($this->BILL_ID->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
