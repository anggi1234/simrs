...
class VRawatInapView extends VRawatInap
{
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("TreatmentBillList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("TreatmentBillGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_RAWAT_INAP')) {
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_RAWAT_INAP')) {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PasienDiagnosaList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_THENAME", $this->THENAME->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_THEADDRESS", $this->THEADDRESS->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        // "detail_OBSTETRI"
        $item = &$option->add("detail_OBSTETRI");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("OBSTETRI", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ObstetriList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_THENAME", $this->THENAME->CurrentValue) . "&" . GetForeignKeyUrl("fk_THEADDRESS", $this->THEADDRESS->CurrentValue) . "&" . GetForeignKeyUrl("fk_GENDER", $this->GENDER->CurrentValue) . "&" . GetForeignKeyUrl("fk_ORG_UNIT_CODE", $this->ORG_UNIT_CODE->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("ObstetriGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_RAWAT_INAP')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=OBSTETRI"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "OBSTETRI";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_RAWAT_INAP')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=OBSTETRI"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "OBSTETRI";
        }
        if ($links != "") {
            $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-toggle=\"dropdown\"></button>";
            $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        $body = "<div class=\"btn-group btn-group-sm ew-btn-group\">" . $body . "</div>";
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'OBSTETRI');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "OBSTETRI";
        }
        if ($this->ShowMultipleDetails) {
            $item->Visible = false;
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("OBSTETRI", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->THEADDRESS->CurrentValue = $this->THEADDRESS->CurrentValue;
                    $detailPageObj->THEADDRESS->setSessionValue($detailPageObj->THEADDRESS->CurrentValue);
                    $detailPageObj->GENDER->IsDetailKey = true;
                    $detailPageObj->GENDER->CurrentValue = $this->GENDER->CurrentValue;
                    $detailPageObj->GENDER->setSessionValue($detailPageObj->GENDER->CurrentValue);
                    $detailPageObj->ORG_UNIT_CODE->IsDetailKey = true;
                    $detailPageObj->ORG_UNIT_CODE->CurrentValue = $this->ORG_UNIT_CODE->CurrentValue;
                    $detailPageObj->ORG_UNIT_CODE->setSessionValue($detailPageObj->ORG_UNIT_CODE->CurrentValue);
                }
            }
        }
    }
