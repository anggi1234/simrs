...
class TreatmentBill extends DbTable
{
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['BILL_ID']) && $rsold['BILL_ID'] != $rs['BILL_ID'])) { // Update detail field 'BILL_ID'
            $cascadeUpdate = true;
            $rscascade['BILL_ID'] = $rs['BILL_ID'];
        }
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("TREATMENT_AKOMODASI")->loadRs("[BILL_ID] = " . QuotedValue($rsold['BILL_ID'], DATATYPE_STRING, 'DB') . " AND " . "[VISIT_ID] = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB') . " AND " . "[NO_REGISTRATION] = " . QuotedValue($rsold['NO_REGISTRATION'], DATATYPE_STRING, 'DB') . " AND " . "[TRANS_ID] = " . QuotedValue($rsold['TRANS_ID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("V_AKOMODASI_KAMAR")->loadRs("dbo.TREATMENT_AKOMODASI.BILL_ID = " . QuotedValue($rsold['BILL_ID'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.VISIT_ID = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.NO_REGISTRATION = " . QuotedValue($rsold['NO_REGISTRATION'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_AKOMODASI.TRANS_ID = " . QuotedValue($rsold['TRANS_ID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'ID';
                $rskey[$fldname] = $rsdtlold[$fldname];
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("TREATMENT_AKOMODASI")->loadRs("[BILL_ID] = " . QuotedValue($rs['BILL_ID'], DATATYPE_STRING, "DB") . " AND " . "[VISIT_ID] = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB") . " AND " . "[NO_REGISTRATION] = " . QuotedValue($rs['NO_REGISTRATION'], DATATYPE_STRING, "DB") . " AND " . "[TRANS_ID] = " . QuotedValue($rs['TRANS_ID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        ...
        $dtlrows = Container("V_AKOMODASI_KAMAR")->loadRs("dbo.TREATMENT_AKOMODASI.BILL_ID = " . QuotedValue($rs['BILL_ID'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.VISIT_ID = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.NO_REGISTRATION = " . QuotedValue($rs['NO_REGISTRATION'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_AKOMODASI.TRANS_ID = " . QuotedValue($rs['TRANS_ID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->amount_paid->EditValue = FormatNumber($this->amount_paid->EditValue, 0, -2, -2, -2);
        $this->amount_paid->ViewCustomAttributes = "";
    ...
    public function exportDocument($doc, $recordset, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->ORG_UNIT_CODE, '');
