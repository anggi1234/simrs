 ...
 * Page class
 */
class VisitReasonAdd extends VisitReason
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VisitReasonAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("VisitReasonList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "VisitReasonList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "VisitReasonView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->LAKALANTAS->CurrentValue = "0";
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'LAKALANTAS' first before field var 'x_LAKALANTAS'
        $val = $CurrentForm->hasValue("LAKALANTAS") ? $CurrentForm->getValue("LAKALANTAS") : $CurrentForm->getValue("x_LAKALANTAS");
        if (!$this->LAKALANTAS->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->LAKALANTAS->Visible = false; // Disable update for API request
            } else {
                $this->LAKALANTAS->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->LAKALANTAS->CurrentValue = $this->LAKALANTAS->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['LAKALANTAS'] = $this->LAKALANTAS->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->LAKALANTAS->EditValue = HtmlEncode($this->LAKALANTAS->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->LAKALANTAS->Required) {
            if (!$this->LAKALANTAS->IsDetailKey && EmptyValue($this->LAKALANTAS->FormValue)) {
                $this->LAKALANTAS->addErrorMessage(str_replace("%s", $this->LAKALANTAS->caption(), $this->LAKALANTAS->RequiredErrorMessage));
            }
        }
    ...
    {
        global $Language, $Security;
        if ($this->REASON_ID->CurrentValue != "") { // Check field with unique index
            $filter = "([REASON_ID] = " . AdjustSql($this->REASON_ID->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->REASON_ID->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->REASON_ID->CurrentValue, $idxErrMsg);
        ...
        // LAKALANTAS
        $this->LAKALANTAS->setDbValueDef($rsnew, $this->LAKALANTAS->CurrentValue, null, strval($this->LAKALANTAS->CurrentValue) == "");

        // Call Row Inserting event
        $insertRow = $this->rowInserting($rsold, $rsnew);

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['REASON_ID']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VisitReasonList"), "", $this->TableVar, true);
