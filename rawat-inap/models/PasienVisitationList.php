...
class PasienVisitationList extends PasienVisitation
{
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_V_REG_AKOMODASI");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'V_REG_AKOMODASI') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'V_AKOMODASI_KAMAR')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("VAkomodasiKamarList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        // "detail_V_REG_AKOMODASI"
        $opt = $this->ListOptions["detail_V_REG_AKOMODASI"];
        if ($Security->allowList(CurrentProjectID() . 'V_REG_AKOMODASI')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("V_REG_AKOMODASI", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("VRegAkomodasiList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("VRegAkomodasiGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'PASIEN_VISITATION')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=V_REG_AKOMODASI");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'PASIEN_VISITATION')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=V_REG_AKOMODASI");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
