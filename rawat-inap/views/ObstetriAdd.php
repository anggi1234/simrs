...
loadjs.ready("head", function () {
    ...
    // Multi-Page
    fOBSTETRIadd.multiPage = new ew.MultiPage("fOBSTETRIadd");

    // Dynamic selection lists
    fOBSTETRIadd.lists.GENDER = <?= $Page->GENDER->toClientList($Page) ?>;
    fOBSTETRIadd.lists.CLINIC_ID = <?= $Page->CLINIC_ID->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BIRTH_NB = <?= $Page->BIRTH_NB->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BIRTH_PLACE = <?= $Page->BIRTH_PLACE->toClientList($Page) ?>;
    fOBSTETRIadd.lists.ANTE_NATAL = <?= $Page->ANTE_NATAL->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BIRTH_WAY = <?= $Page->BIRTH_WAY->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BIRTH_BY = <?= $Page->BIRTH_BY->toClientList($Page) ?>;
    fOBSTETRIadd.lists.ABORTUS_ID = <?= $Page->ABORTUS_ID->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BIRTH_CON = <?= $Page->BIRTH_CON->toClientList($Page) ?>;
    fOBSTETRIadd.lists.FOLLOW_UP = <?= $Page->FOLLOW_UP->toClientList($Page) ?>;
    fOBSTETRIadd.lists.DIRUJUK_OLEH = <?= $Page->DIRUJUK_OLEH->toClientList($Page) ?>;
    fOBSTETRIadd.lists.RAHIM_ID = <?= $Page->RAHIM_ID->toClientList($Page) ?>;
    fOBSTETRIadd.lists.BLOODING = <?= $Page->BLOODING->toClientList($Page) ?>;
...
<input type="hidden" name="fk_THENAME" value="<?= HtmlEncode($Page->THENAME->getSessionValue()) ?>">
<input type="hidden" name="fk_THEADDRESS" value="<?= HtmlEncode($Page->THEADDRESS->getSessionValue()) ?>">
<input type="hidden" name="fk_GENDER" value="<?= HtmlEncode($Page->GENDER->getSessionValue()) ?>">
<input type="hidden" name="fk_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->ORG_UNIT_CODE->getSessionValue()) ?>">
<?php } ?>
...
<?php } else { ?>
<span id="el_OBSTETRI_THENAME">
<input type="<?= $Page->THENAME->getInputTextType() ?>" data-table="OBSTETRI" data-field="x_THENAME" data-page="1" name="x_THENAME" id="x_THENAME" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->THENAME->getPlaceHolder()) ?>" value="<?= $Page->THENAME->EditValue ?>"<?= $Page->THENAME->editAttributes() ?> aria-describedby="x_THENAME_help">
...
<?php if ($Page->GENDER->getSessionValue() != "") { ?>
...
<input type="hidden" id="x_GENDER" name="x_GENDER" value="<?= HtmlEncode($Page->GENDER->CurrentValue) ?>" data-hidden="1">
<?php } else { ?>
<span id="el_OBSTETRI_GENDER">
    <select
        id="x_GENDER"
        ...
        data-select2-id="OBSTETRI_x_GENDER"
        data-table="OBSTETRI"
        data-field="x_GENDER"
        data-page="1"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='OBSTETRI_x_GENDER']"),
        options = { name: "x_GENDER", selectId: "OBSTETRI_x_GENDER", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
...
<span id="el_OBSTETRI_CLINIC_ID">
    <select
        ...
        data-select2-id="OBSTETRI_x_CLINIC_ID"
        data-table="OBSTETRI"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='OBSTETRI_x_CLINIC_ID']"),
        options = { name: "x_CLINIC_ID", selectId: "OBSTETRI_x_CLINIC_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
...
loadjs.ready(["fOBSTETRIadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fOBSTETRIadd", "x_BIRTH_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fOBSTETRIadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fOBSTETRIadd", "x_INSPECTION_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":11});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <?php if (strval($Page->ORG_UNIT_CODE->getSessionValue()) != "") { ?>
    <input type="hidden" name="x_ORG_UNIT_CODE" id="x_ORG_UNIT_CODE" value="<?= HtmlEncode(strval($Page->ORG_UNIT_CODE->getSessionValue())) ?>">
    <?php } ?>
