...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->MUTATION_BY->getDisplayValue($Page->MUTATION_BY->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="MUTATION_DOCS" data-field="x_MUTATION_BY" data-hidden="1" name="x_MUTATION_BY" id="x_MUTATION_BY" value="<?= HtmlEncode($Page->MUTATION_BY->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID" class="form-group row">
        <label id="elh_MUTATION_DOCS_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
