...
var fUSER_ACCESSedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fUSER_ACCESSedit = currentForm = new ew.Form("fUSER_ACCESSedit", "edit");
    ...
    // Validate form
    fUSER_ACCESSedit.validate = function () {
    ...
    // Form_CustomValidate
    fUSER_ACCESSedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fUSER_ACCESSedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fUSER_ACCESSedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fUSER_ACCESSedit" id="fUSER_ACCESSedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="USER_ACCESS" data-field="x_ORG_UNIT_CODE" data-hidden="1" name="o_ORG_UNIT_CODE" id="o_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->ORG_UNIT_CODE->OldValue ?? $Page->ORG_UNIT_CODE->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="USER_ACCESS" data-field="x__USERNAME" data-hidden="1" name="o__USERNAME" id="o__USERNAME" value="<?= HtmlEncode($Page->_USERNAME->OldValue ?? $Page->_USERNAME->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="USER_ACCESS" data-field="x_USE_ORG_UNIT_CODE" data-hidden="1" name="o_USE_ORG_UNIT_CODE" id="o_USE_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->USE_ORG_UNIT_CODE->OldValue ?? $Page->USE_ORG_UNIT_CODE->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="USER_ACCESS" data-field="x_GROUP_ID" data-hidden="1" name="o_GROUP_ID" id="o_GROUP_ID" value="<?= HtmlEncode($Page->GROUP_ID->OldValue ?? $Page->GROUP_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fUSER_ACCESSedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fUSER_ACCESSedit", "x_MODIFIED_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
