...
loadjs.ready("head", function () {
    ...
    fV_AKOMODASI_KAMARadd.lists.CLINIC_ID = <?= $Page->CLINIC_ID->toClientList($Page) ?>;
    fV_AKOMODASI_KAMARadd.lists.CLASS_ROOM_ID = <?= $Page->CLASS_ROOM_ID->toClientList($Page) ?>;
    fV_AKOMODASI_KAMARadd.lists.KELUAR_ID = <?= $Page->KELUAR_ID->toClientList($Page) ?>;
    fV_AKOMODASI_KAMARadd.lists.BED_ID = <?= $Page->BED_ID->toClientList($Page) ?>;
    fV_AKOMODASI_KAMARadd.lists.EMPLOYEE_ID = <?= $Page->EMPLOYEE_ID->toClientList($Page) ?>;
...
<span id="el_V_AKOMODASI_KAMAR_CLINIC_ID">
    <select
        ...
        data-select2-id="V_AKOMODASI_KAMAR_x_CLINIC_ID"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='V_AKOMODASI_KAMAR_x_CLINIC_ID']"),
        options = { name: "x_CLINIC_ID", selectId: "V_AKOMODASI_KAMAR_x_CLINIC_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
...
<?php if ($Page->TREAT_DATE->Visible) { // TREAT_DATE ?>
    <div id="r_TREAT_DATE" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_TREAT_DATE" for="x_TREAT_DATE" class="<?= $Page->LeftColumnClass ?>"><?= $Page->TREAT_DATE->caption() ?><?= $Page->TREAT_DATE->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->TREAT_DATE->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_TREAT_DATE">
<input type="<?= $Page->TREAT_DATE->getInputTextType() ?>" data-table="V_AKOMODASI_KAMAR" data-field="x_TREAT_DATE" data-format="11" name="x_TREAT_DATE" id="x_TREAT_DATE" placeholder="<?= HtmlEncode($Page->TREAT_DATE->getPlaceHolder()) ?>" value="<?= $Page->TREAT_DATE->EditValue ?>"<?= $Page->TREAT_DATE->editAttributes() ?> aria-describedby="x_TREAT_DATE_help">
...
<script>
loadjs.ready(["fV_AKOMODASI_KAMARadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fV_AKOMODASI_KAMARadd", "x_TREAT_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":11});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->DESCRIPTION->Visible) { // DESCRIPTION ?>
    <div id="r_DESCRIPTION" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_DESCRIPTION" for="x_DESCRIPTION" class="<?= $Page->LeftColumnClass ?>"><?= $Page->DESCRIPTION->caption() ?><?= $Page->DESCRIPTION->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->DESCRIPTION->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_DESCRIPTION">
<input type="<?= $Page->DESCRIPTION->getInputTextType() ?>" data-table="V_AKOMODASI_KAMAR" data-field="x_DESCRIPTION" name="x_DESCRIPTION" id="x_DESCRIPTION" size="30" maxlength="200" placeholder="<?= HtmlEncode($Page->DESCRIPTION->getPlaceHolder()) ?>" value="<?= $Page->DESCRIPTION->EditValue ?>"<?= $Page->DESCRIPTION->editAttributes() ?> aria-describedby="x_DESCRIPTION_help">
...
<?php if ($Page->CLASS_ROOM_ID->Visible) { // CLASS_ROOM_ID ?>
    <div id="r_CLASS_ROOM_ID" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_CLASS_ROOM_ID" for="x_CLASS_ROOM_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->CLASS_ROOM_ID->caption() ?><?= $Page->CLASS_ROOM_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->CLASS_ROOM_ID->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_CLASS_ROOM_ID">
<?php $Page->CLASS_ROOM_ID->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
...
<?php if ($Page->KELUAR_ID->Visible) { // KELUAR_ID ?>
    <div id="r_KELUAR_ID" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_KELUAR_ID" for="x_KELUAR_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->KELUAR_ID->caption() ?><?= $Page->KELUAR_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->KELUAR_ID->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_KELUAR_ID">
    <select
        id="x_KELUAR_ID"
        name="x_KELUAR_ID"
        class="form-control ew-select<?= $Page->KELUAR_ID->isInvalidClass() ?>"
        data-select2-id="V_AKOMODASI_KAMAR_x_KELUAR_ID"
        data-table="V_AKOMODASI_KAMAR"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='V_AKOMODASI_KAMAR_x_KELUAR_ID']"),
        options = { name: "x_KELUAR_ID", selectId: "V_AKOMODASI_KAMAR_x_KELUAR_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
...
<?php if ($Page->BED_ID->Visible) { // BED_ID ?>
    <div id="r_BED_ID" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_BED_ID" for="x_BED_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->BED_ID->caption() ?><?= $Page->BED_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->BED_ID->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_BED_ID">
    <select
        id="x_BED_ID"
        name="x_BED_ID"
        class="form-control ew-select<?= $Page->BED_ID->isInvalidClass() ?>"
        data-select2-id="V_AKOMODASI_KAMAR_x_BED_ID"
        data-table="V_AKOMODASI_KAMAR"
        data-field="x_BED_ID"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='V_AKOMODASI_KAMAR_x_BED_ID']"),
        options = { name: "x_BED_ID", selectId: "V_AKOMODASI_KAMAR_x_BED_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
...
<?php if ($Page->EMPLOYEE_ID->Visible) { // EMPLOYEE_ID ?>
    <div id="r_EMPLOYEE_ID" class="form-group row">
        <label id="elh_V_AKOMODASI_KAMAR_EMPLOYEE_ID" for="x_EMPLOYEE_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->EMPLOYEE_ID->caption() ?><?= $Page->EMPLOYEE_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->EMPLOYEE_ID->cellAttributes() ?>>
<span id="el_V_AKOMODASI_KAMAR_EMPLOYEE_ID">
    <select
        id="x_EMPLOYEE_ID"
        name="x_EMPLOYEE_ID"
        class="form-control ew-select<?= $Page->EMPLOYEE_ID->isInvalidClass() ?>"
        data-select2-id="V_AKOMODASI_KAMAR_x_EMPLOYEE_ID"
        data-table="V_AKOMODASI_KAMAR"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='V_AKOMODASI_KAMAR_x_EMPLOYEE_ID']"),
        options = { name: "x_EMPLOYEE_ID", selectId: "V_AKOMODASI_KAMAR_x_EMPLOYEE_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
