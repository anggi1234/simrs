...
loadjs.ready("head", function () {
    ...
    fWEEK_DAYSedit = currentForm = new ew.Form("fWEEK_DAYSedit", "edit");
    ...
    // Validate form
    fWEEK_DAYSedit.validate = function () {
    ...
    // Form_CustomValidate
    fWEEK_DAYSedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fWEEK_DAYSedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fWEEK_DAYSedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fWEEK_DAYSedit" id="fWEEK_DAYSedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="WEEK_DAYS" data-field="x_DAY_ID" data-hidden="1" name="o_DAY_ID" id="o_DAY_ID" value="<?= HtmlEncode($Page->DAY_ID->OldValue ?? $Page->DAY_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
