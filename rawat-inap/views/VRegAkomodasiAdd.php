...
// Page object
$VRegAkomodasiAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fV_REG_AKOMODASIadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fV_REG_AKOMODASIadd = currentForm = new ew.Form("fV_REG_AKOMODASIadd", "add");
    ...
    // Validate form
    fV_REG_AKOMODASIadd.validate = function () {
    ...
    // Form_CustomValidate
    fV_REG_AKOMODASIadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fV_REG_AKOMODASIadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fV_REG_AKOMODASIadd.lists.CLINIC_ID = <?= $Page->CLINIC_ID->toClientList($Page) ?>;
    fV_REG_AKOMODASIadd.lists.BED_ID = <?= $Page->BED_ID->toClientList($Page) ?>;
    fV_REG_AKOMODASIadd.lists.KELUAR_ID = <?= $Page->KELUAR_ID->toClientList($Page) ?>;
    fV_REG_AKOMODASIadd.lists.CLASS_ROOM_ID = <?= $Page->CLASS_ROOM_ID->toClientList($Page) ?>;
    fV_REG_AKOMODASIadd.lists.EMPLOYEE_ID = <?= $Page->EMPLOYEE_ID->toClientList($Page) ?>;
    loadjs.done("fV_REG_AKOMODASIadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fV_REG_AKOMODASIadd" id="fV_REG_AKOMODASIadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_V_REG_AKOMODASI_CLINIC_ID">
    <select
        ...
        data-select2-id="V_REG_AKOMODASI_x_CLINIC_ID"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='V_REG_AKOMODASI_x_CLINIC_ID']"),
        options = { name: "x_CLINIC_ID", selectId: "V_REG_AKOMODASI_x_CLINIC_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
...
<input type="<?= $Page->TREATMENT->getInputTextType() ?>" data-table="V_REG_AKOMODASI" data-field="x_TREATMENT" name="x_TREATMENT" id="x_TREATMENT" size="30" maxlength="200" placeholder="<?= HtmlEncode($Page->TREATMENT->getPlaceHolder()) ?>" value="<?= $Page->TREATMENT->EditValue ?>"<?= $Page->TREATMENT->editAttributes() ?> aria-describedby="x_TREATMENT_help">
...
<input type="<?= $Page->TREAT_DATE->getInputTextType() ?>" data-table="V_REG_AKOMODASI" data-field="x_TREAT_DATE" name="x_TREAT_DATE" id="x_TREAT_DATE" placeholder="<?= HtmlEncode($Page->TREAT_DATE->getPlaceHolder()) ?>" value="<?= $Page->TREAT_DATE->EditValue ?>"<?= $Page->TREAT_DATE->editAttributes() ?> aria-describedby="x_TREAT_DATE_help">
...
<script>
loadjs.ready(["fV_REG_AKOMODASIadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fV_REG_AKOMODASIadd", "x_TREAT_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
