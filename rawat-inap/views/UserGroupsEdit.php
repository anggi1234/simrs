...
var fUSER_GROUPSedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fUSER_GROUPSedit = currentForm = new ew.Form("fUSER_GROUPSedit", "edit");
    ...
    // Form_CustomValidate
    fUSER_GROUPSedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fUSER_GROUPSedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fUSER_GROUPSedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fUSER_GROUPSedit" id="fUSER_GROUPSedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="USER_GROUPS" data-field="x_ORG_UNIT_CODE" data-hidden="1" name="o_ORG_UNIT_CODE" id="o_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->ORG_UNIT_CODE->OldValue ?? $Page->ORG_UNIT_CODE->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="USER_GROUPS" data-field="x_GROUP_ID" data-hidden="1" name="o_GROUP_ID" id="o_GROUP_ID" value="<?= HtmlEncode($Page->GROUP_ID->OldValue ?? $Page->GROUP_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fUSER_GROUPSedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fUSER_GROUPSedit", "x_MODIFIED_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
