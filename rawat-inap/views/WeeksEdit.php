...
loadjs.ready("head", function () {
    ...
    fWEEKSedit = currentForm = new ew.Form("fWEEKSedit", "edit");
    ...
    // Form_CustomValidate
    fWEEKSedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fWEEKSedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fWEEKSedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fWEEKSedit" id="fWEEKSedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="WEEKS" data-field="x_WEEK_ID" data-hidden="1" name="o_WEEK_ID" id="o_WEEK_ID" value="<?= HtmlEncode($Page->WEEK_ID->OldValue ?? $Page->WEEK_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
