...
$sqlitemanamnesa = "SELECT
    ...
    CONCAT('<strong>S</strong> : ',
        ...
        SEPARATOR ', ') AS diagnosa
                        FROM
                            data_penyakit o
                                LEFT JOIN
                            master_penyakit b ON o.id_penyakit = b.id_master_penyakit
                                LEFT JOIN
                            data c ON o.idxdaftar = c.idxdaftar
                                LEFT JOIN
                            icd_eklaim d ON o.icd10 = d.code
                        WHERE
                            o.idxdaftar = a.idxdaftar
                        GROUP BY o.idxdaftar),
                    ''),
            '<br><br>',
            ...
            IFNULL((SELECT
                        ...
                        WHERE
                            z.idxdaftar = a.idxdaftar group by z.idxdaftar),
                    ''),
            ',',
            IFNULL((SELECT
                            group_concat(CASE
                        ...
                        WHERE
                            z.idxdaftar = a.idxdaftar
                                AND z.id_status_keluar = 5 group by z.idxdaftar),
    ...
    CONCAT((ifnull((SELECT
                ...
                FROM
                    data_tindakan o
                        LEFT JOIN
                    master_tindakan b ON o.id_tindakan = b.id_tindakan
                WHERE
                    b.userlevelid IS NULL
                        AND o.idxdaftar = a.idxdaftar
                GROUP BY o.idxdaftar),'')),
            ' | ',
            (SELECT
            ...
            '') AS obat
                FROM
                    bill_detail_permintaan_obat o
                        LEFT JOIN
                    master_obat_detail b ON o.id_master_obat_detail = b.id_master_obat_detail
                        LEFT JOIN
                    master_obat c ON b.id_obat = c.id_obat
                        LEFT JOIN
                    bill_detail_permintaan_obat_master d ON o.id_bill_detail_permintaan_obat_master = d.id_bill_detail_permintaan_obat_master
                        LEFT JOIN
                    l_jenis_racikan e ON o.id_jenis_racikan = e.id_jenis_racikan
                WHERE
                    o.idxdaftar = a.idxdaftar
                ...
                GROUP BY o.idxdaftar)) AS intruksi,
    h.signature,
    CONCAT('<br><center>',
            h.pd_nickname,
            '<br>',
...
FROM
    ...
    userlevels g ON b.userlevelid = g.userlevelid
        LEFT JOIN
    master_login h ON b.kddokter = h.kddokter
        LEFT JOIN
    master_profesi i ON h.id_profesi = i.id_profesi
        LEFT JOIN
    master_profesi_sub j ON h.id_profesi_sub = j.id_master_profesi_sub
WHERE
...
$sqlumur = "SELECT
    nomr,
    CONCAT(TIMESTAMPDIFF(YEAR,pasien_tgl_lahir,NOW()),' Tahun ',(TIMESTAMPDIFF(MONTH,pasien_tgl_lahir,NOW()) % 12),' Bulan') as umur
FROM
    data
WHERE
    nomr = $nomr";
$queryumur = mysqli_query($mysqli, $sqlumur);
$DATAUMUR  = mysqli_fetch_array($queryumur);
  ...
  <tbody>
    <tr>
      <td width="33%"><table width="100%" border="0">
        <tbody>
          <tr>
            <td width="23%" align="center"><img src="http://localhost/simrs/cetak/gambar/logorsud.png" height="60px" /></td>
...
<table width="100%" border="0" class="tabel">
 ...
 dengan rinci dan jelas)
	  ...
	  <?php
while ($data1 = mysqli_fetch_assoc($queryitemanamnesa)) {
    echo '<tr>';
    ...
    echo '<td style="font-size: 8px; padding-bottom:20px;" valign="button" class="kosong"><center><img src="" height="40px" /></center> ' . $data1['dpjp'] . '</td>';
    echo '</tr>';
}
