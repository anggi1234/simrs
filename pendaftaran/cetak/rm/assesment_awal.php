...
FROM
    ...
    data b ON a.idxdaftar = b.idxdaftar
        LEFT JOIN
    m_pasien d ON b.NOMR = d.NOMR
        LEFT JOIN
    l_skala_nyeri f ON a.skala_nyeri = f.id_skala_nyeri
        LEFT JOIN
    userlevels g ON b.userlevelid = g.userlevelid
WHERE
    a.idxdaftar = $idxdaftar";
$queryitemanamnesa = mysqli_query($mysqli, $sqlitemanamnesa);
$DATAISI           = mysqli_fetch_array($queryitemanamnesa);
...
$sqlitemobjektif = "SELECT
    ...
    rencana_tindakan,
    mata_visus_dasar_od,
    mata_visus_koreksi_od,
    mata_tekanan_intraokuler_od,
    mata_palpebra_od,
    mata_konjungtiva_od,
    mata_kornea_od,
    mata_coa_od,
    ...
    mata_funduskopi_od,
    mata_visus_dasar_os,
    mata_visus_koreksi_os,
    mata_tekanan_intraokuler_os,
    mata_palpebra_os,
    mata_konjungtiva_os,
    mata_kornea_os,
    mata_coa_os,
    ...
    jiwa_insight,
    saraf_meningealsign,
    saraf_nervus_cranialis,
    saraf_motorik,
    saraf_sensorik,
    saraf_otonom,
    bidan_fundus_uteri,
    bidan_denyut_janin,
    bidan_vaginal_toucher,
    bidan_letak_janin,
    bidan_status_obstetri,
    bidan_his
FROM
    data_keterangan
	where idxdaftar = $idxdaftar";
$queryitemobjektif = mysqli_query($mysqli, $sqlitemobjektif);
$DATAOBJEKTIF      = mysqli_fetch_array($queryitemobjektif);
...
FROM
    data_penyakit a
        LEFT JOIN
    icd_eklaim b ON a.icd10 = b.code
WHERE
    idxdaftar = $idxdaftar AND a.jenis_diagnosa='PRIMER'
    group by a.jenis_diagnosa";
$queryitemdiagnosa1 = mysqli_query($mysqli, $sqlitemdiagnosa1);
$DATADIAGNOSAPRIMER = mysqli_fetch_array($queryitemdiagnosa1);

$sqlitemdiagnosa2 = "SELECT
    ifnull(group_concat(CONCAT(CODE, ', ', STR) separator ' | '),'') AS diagnosa
FROM
    data_penyakit a
        LEFT JOIN
    icd_eklaim b ON a.icd10 = b.code
WHERE
    idxdaftar = $idxdaftar AND a.jenis_diagnosa='SEKUNDER'
    -- group by a.jenis_diagnosa";
$queryitemdiagnosa2   = mysqli_query($mysqli, $sqlitemdiagnosa2);
$DATADIAGNOSASEKUNDER = mysqli_fetch_array($queryitemdiagnosa2);
...
$DATAOBAT      = mysqli_fetch_array($queryitemobat);

$sqlitempenunjang = "SELECT
    b.userlevelname, a.nama_order
FROM
    data_order_penunjang a
        LEFT JOIN
    userlevels b ON a.userlevelid_tujuan = b.userlevelid
    where idxdaftar=$idxdaftar";
$queryitempenunjang = mysqli_query($mysqli, $sqlitempenunjang);
...
$DATATINDAKAN      = mysqli_fetch_array($queryitemtindakan);
// urung dadi sql le
...
WHERE
    b.nomr = $nomr and b.idxdaftar < $idxdaftar
	and d.nama_obat NOT LIKE '%JASA%'
		and d.nama_obat NOT LIKE '%SPUIT%'
		and d.nama_obat NOT LIKE '%INJEKSI%'
		and d.nama_obat NOT LIKE '%KASSA%'
		...
		and d.nama_obat NOT LIKE '%INJ%'
GROUP BY c.id_obat";
$queryitemobatdikonsumsi = mysqli_query($mysqli, $sqlitemobatdikonsumsi);
...
FROM
    data_keterangan a
        LEFT JOIN
    master_login b ON a.username_dokter = b.username
    where a.idxdaftar=$idxdaftar";
$queryitemdokter = mysqli_query($mysqli, $sqlitemdokter);
$DATADOKTER      = mysqli_fetch_array($queryitemdokter);
...
FROM
    data_keterangan a
        LEFT JOIN
    master_login b ON a.username_perawat = b.username
    where a.idxdaftar=$idxdaftar";
$queryitemperawat = mysqli_query($mysqli, $sqlitemperawat);
$DATAPERAWAT      = mysqli_fetch_array($queryitemperawat);
...
WHERE a.idxdaftar = '$idxdaftar'";
$queryitemusername = mysqli_query($mysqli, $sqlitemusername);
$DATA_IDENTITAS    = mysqli_fetch_array($queryitemusername);
      ...
      <td colspan="6"><table width="100%" class="tabel" border="1">
        <tbody>
          <tr>
			...
			<?php
$no = 1;
while ($data = mysqli_fetch_assoc($queryitemobatdikonsumsi)) {
    echo '<tr>';
    echo '<td>' . $no . '</td>';
...
FROM
    ...
    FROM
        data_keterangan
    WHERE
        idxdaftar = 164078 UNION ALL SELECT
    ...
    FROM
        data_keterangan
    WHERE
        idxdaftar = $idxdaftar) s;";
$queryitempemeriksaan = mysqli_query($mysqli, $sqlitempemeriksaan);
?>
    <tr>
      <td>&nbsp;</td>
      ...
      <td colspan="4">
        ...
        <tbody>
          <?php
while ($data = mysqli_fetch_assoc($queryitempemeriksaan)) {
    echo '<tr>';
    ...
    <tr>
      ...
      <td colspan="4" valign="top"><table width="100%" class="tabel" border="1">
        <tbody>
          ...
          <?php
while ($data = mysqli_fetch_assoc($queryitempenunjang)) {
    echo '<tr>';
    ...
    <tr>
      <td colspan="7"><table width="100%" border="0">
        <tbody>
          ...
          <tr>
            <td align="center">&nbsp;</td>
            <td rowspan="3" align="center"><img src="" height="70px" width="70px" /></td>
            <td align="center">&nbsp;</td>
            <td rowspan="3" align="center"><img src="" width="70px" height="70px" /></td>
          </tr>
          <tr>
            <td align="center">&nbsp;</td>
            <td align="center">&nbsp;</td>
...
<div style="font-size: 9px; font-style: oblique">SIMRS VERSI 3 RSUD Besemah Pagar Alam</div>
