...
<table id="tbl_PASIEN_VISITATIONmaster" class="table ew-view-table ew-master-table ew-vertical">
    <tbody>
<?php if ($PASIEN_VISITATION->SEP->Visible) { // SEP ?>
        <tr id="r_SEP">
            <td class="<?= $PASIEN_VISITATION->TableLeftColumnClass ?>"><?= $PASIEN_VISITATION->SEP->caption() ?></td>
            <td <?= $PASIEN_VISITATION->SEP->cellAttributes() ?>>
<span id="el_PASIEN_VISITATION_SEP">
<span<?= $PASIEN_VISITATION->SEP->viewAttributes() ?>>
<?= $PASIEN_VISITATION->SEP->getViewValue() ?></span>
</span>
</td>
        </tr>
<?php } ?>
