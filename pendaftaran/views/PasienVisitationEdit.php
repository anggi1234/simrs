...
<?php if ($Page->COVERAGE_ID->Visible) { // COVERAGE_ID ?>
    <div id="r_COVERAGE_ID" class="form-group row">
        <label id="elh_PASIEN_VISITATION_COVERAGE_ID" for="x_COVERAGE_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->COVERAGE_ID->caption() ?><?= $Page->COVERAGE_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_PASIEN_VISITATION_COVERAGE_ID">
    <select
        ...
        data-select2-id="PASIEN_VISITATION_x_COVERAGE_ID"
        data-table="PASIEN_VISITATION"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='PASIEN_VISITATION_x_COVERAGE_ID']"),
        options = { name: "x_COVERAGE_ID", selectId: "PASIEN_VISITATION_x_COVERAGE_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.PASIEN_VISITATION.fields.COVERAGE_ID.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
...
?>
<a href="../bridging/getrujukan.php?key=<?php echo urlencode(CurrentPage()->PASIEN_ID->CurrentValue).'&id='.urlencode(CurrentPage()->IDXDAFTAR->CurrentValue).'&no='.urlencode(CurrentPage()->NO_REGISTRATION->CurrentValue)?>" class="btn btn-info btn-sm" role="button">Ambil Rujukan</a>
...
loadjs.ready("load", function () {
    // Startup script
    function Buka(n=""){return window.open(n,"newwindow","width=800,height=400"),!1}var visit="../bridging/jasper.php?tipe=ANTRIAN_POLI&id="+document.getElementById("x_VISIT_ID").value;console.log(visit),$("#el_PASIEN_VISITATION_TICKET_NO").append('<a href="#" id="url" class="btn btn-info btn-sm" role="button">Cetak</a>'),document.getElementById("url").onclick=function(){Buka(visit)};
});
</script>
