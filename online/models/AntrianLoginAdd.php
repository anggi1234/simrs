...
class AntrianLoginAdd extends AntrianLogin
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->JENIS_KELAMIN->ViewValue = $this->JENIS_KELAMIN->Lookup !== null && is_array($this->JENIS_KELAMIN->Lookup->Options) ? $curVal : null;
            }
            if ($this->JENIS_KELAMIN->ViewValue !== null) { // Load from cache
                $this->JENIS_KELAMIN->EditValue = array_values($this->JENIS_KELAMIN->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[GENDER]" . SearchString("=", $this->JENIS_KELAMIN->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->JENIS_KELAMIN->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->JENIS_KELAMIN->EditValue = $arwrk;
            }
            ...
            if ($this->AGAMA->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "[KODE_AGAMA]" . SearchString("=", $this->AGAMA->CurrentValue, DATATYPE_NUMBER, "");
                }
            ...
            if ($curVal != "") {
                ...
                $this->PEKERJAAN->ViewValue = $this->PEKERJAAN->Lookup !== null && is_array($this->PEKERJAAN->Lookup->Options) ? $curVal : null;
            }
            if ($this->PEKERJAAN->ViewValue !== null) { // Load from cache
                $this->PEKERJAAN->EditValue = array_values($this->PEKERJAAN->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[JOB_ID]" . SearchString("=", $this->PEKERJAAN->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->PEKERJAAN->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->PEKERJAAN->EditValue = $arwrk;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // TANGGAL_REGIS
        $this->TANGGAL_REGIS->CurrentValue = CurrentDate();
        $this->TANGGAL_REGIS->setDbValueDef($rsnew, $this->TANGGAL_REGIS->CurrentValue, null);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $Breadcrumb = new Breadcrumb("AntrianLoginList");
        $url = CurrentUrl();
