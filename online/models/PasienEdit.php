...
class PasienEdit extends Pasien
{
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->TMT->IsDetailKey) {
            ...
            $this->TMT->CurrentValue = UnFormatDateTime($this->TMT->CurrentValue, 0);
        }
        ...
        if (!$this->TAT->IsDetailKey) {
            ...
            $this->TAT->CurrentValue = UnFormatDateTime($this->TAT->CurrentValue, 0);
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->TMT->EditValue = HtmlEncode(FormatDateTime($this->TMT->CurrentValue, 8));
            $this->TMT->PlaceHolder = RemoveHtml($this->TMT->caption());

            // TAT
            $this->TAT->EditAttrs["class"] = "form-control";
            $this->TAT->EditCustomAttributes = "";
            $this->TAT->EditValue = HtmlEncode(FormatDateTime($this->TAT->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->TMT->FormValue)) {
        ...
        if (!CheckDate($this->TAT->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // TMT
            $this->TMT->setDbValueDef($rsnew, UnFormatDateTime($this->TMT->CurrentValue, 0), null, $this->TMT->ReadOnly);

            // TAT
            $this->TAT->setDbValueDef($rsnew, UnFormatDateTime($this->TAT->CurrentValue, 0), null, $this->TAT->ReadOnly);
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "PASIEN_VISITATION") {
                ...
                if (($parm = Get("fk_NO_REGISTRATION", Get("NAME_OF_PASIEN"))) !== null) {
                    $masterTbl->NO_REGISTRATION->setQueryStringValue($parm);
                    $this->NAME_OF_PASIEN->setQueryStringValue($masterTbl->NO_REGISTRATION->QueryStringValue);
                    $this->NAME_OF_PASIEN->setSessionValue($this->NAME_OF_PASIEN->QueryStringValue);
                } else {
                    $validMaster = false;
                }
            }
            ...
            if ($masterTblVar == "PASIEN_VISITATION") {
                $validMaster = true;
                $masterTbl = Container("PASIEN_VISITATION");
                if (($parm = Post("fk_NO_REGISTRATION", Post("NAME_OF_PASIEN"))) !== null) {
                    $masterTbl->NO_REGISTRATION->setFormValue($parm);
                    $this->NAME_OF_PASIEN->setFormValue($masterTbl->NO_REGISTRATION->FormValue);
                    $this->NAME_OF_PASIEN->setSessionValue($this->NAME_OF_PASIEN->FormValue);
                } else {
                    $validMaster = false;
                }
            }
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "PASIEN_VISITATION") {
                if ($this->NAME_OF_PASIEN->CurrentValue == "") {
