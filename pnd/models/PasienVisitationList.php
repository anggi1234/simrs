...
class PasienVisitationList extends PasienVisitation
{
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_CV_PASIEN"
        $item = &$this->ListOptions->add("detail_CV_PASIEN");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'CV_PASIEN') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_OBAT')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentObatList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_AKOMODASI')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentAkomodasiList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        // "detail_CV_PASIEN"
        $opt = $this->ListOptions["detail_CV_PASIEN"];
        if ($Security->allowList(CurrentProjectID() . 'CV_PASIEN')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("CV_PASIEN", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("CvPasienList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("CvPasienGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'PASIEN_VISITATION')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=CV_PASIEN");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'PASIEN_VISITATION')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=CV_PASIEN");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item = &$option->add("detailadd_CV_PASIEN");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=CV_PASIEN");
                $detailPage = Container("CvPasienGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'PASIEN_VISITATION') && $Security->canAdd());
