...
class PasienVisitationEdit extends PasienVisitation
{
    ...
    public function run()
    {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                ...
                break;
            case "update": // Update
                $returnUrl = 'PasienVisitationView/'.urlencode(CurrentPage()->IDXDAFTAR->CurrentValue).'?showdetail=';
    ...
    protected function validateForm()
    {
        ...
        if (in_array("CV_PASIEN", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        ...
        if (!$rsold) {
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    $detailPage = Container("CvPasienGrid");
                    if (in_array("CV_PASIEN", $detailTblVar) && $detailPage->DetailEdit) {
                        $Security->loadCurrentUserLevel($this->ProjectID . "CV_PASIEN"); // Load user level of detail table
                        $editRow = $detailPage->gridUpdate();
                        $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                    }
                }
