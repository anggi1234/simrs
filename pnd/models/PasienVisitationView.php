...
class PasienVisitationView extends PasienVisitation
{
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("TreatmentObatList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("TreatmentAkomodasiList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        // "detail_CV_PASIEN"
        $item = &$option->add("detail_CV_PASIEN");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("CV_PASIEN", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("CvPasienList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("CvPasienGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'PASIEN_VISITATION')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=CV_PASIEN"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "CV_PASIEN";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'PASIEN_VISITATION')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=CV_PASIEN"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "CV_PASIEN";
        }
        if ($links != "") {
            $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-toggle=\"dropdown\"></button>";
            $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        $body = "<div class=\"btn-group btn-group-sm ew-btn-group\">" . $body . "</div>";
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'CV_PASIEN');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "CV_PASIEN";
        }
        if ($this->ShowMultipleDetails) {
            $item->Visible = false;
        }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("CV_PASIEN", $detailTblVar)) {
                $detailPageObj = Container("CvPasienGrid");
                if ($detailPageObj->DetailView) {
                    $detailPageObj->CurrentMode = "view";
