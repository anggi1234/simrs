...
loadjs.ready("head", function () {
    ...
    fPASIEN_VISITATIONedit.lists.RUJUKAN_ID = <?= $Page->RUJUKAN_ID->toClientList($Page) ?>;
    ...
    fPASIEN_VISITATIONedit.lists.PAYOR_ID = <?= $Page->PAYOR_ID->toClientList($Page) ?>;
    ...
    fPASIEN_VISITATIONedit.lists.DIAGNOSA_ID = <?= $Page->DIAGNOSA_ID->toClientList($Page) ?>;
    fPASIEN_VISITATIONedit.lists.PPKRUJUKAN = <?= $Page->PPKRUJUKAN->toClientList($Page) ?>;
...
<input type="<?= $Page->DIAG_AWAL->getInputTextType() ?>" data-table="PASIEN_VISITATION" data-field="x_DIAG_AWAL" name="x_DIAG_AWAL" id="x_DIAG_AWAL" size="30" maxlength="10" placeholder="<?= HtmlEncode($Page->DIAG_AWAL->getPlaceHolder()) ?>" value="<?= $Page->DIAG_AWAL->EditValue ?>"<?= $Page->DIAG_AWAL->editAttributes() ?> aria-describedby="x_DIAG_AWAL_help">
...
?>
<?php
    if (in_array("CV_PASIEN", explode(",", $Page->getCurrentDetailTable())) && $CV_PASIEN->DetailEdit) {
