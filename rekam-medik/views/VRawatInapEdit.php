...
<input type="<?= $Page->EXIT_DATE->getInputTextType() ?>" data-table="V_RAWAT_INAP" data-field="x_EXIT_DATE" name="x_EXIT_DATE" id="x_EXIT_DATE" placeholder="<?= HtmlEncode($Page->EXIT_DATE->getPlaceHolder()) ?>" value="<?= $Page->EXIT_DATE->EditValue ?>"<?= $Page->EXIT_DATE->editAttributes() ?> aria-describedby="x_EXIT_DATE_help">
...
loadjs.ready(["fV_RAWAT_INAPedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fV_RAWAT_INAPedit", "x_EXIT_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<input type="hidden" data-table="V_RAWAT_INAP" data-field="x_DIAGNOSA_ID" data-hidden="1" name="x_DIAGNOSA_ID" id="x_DIAGNOSA_ID" value="<?= HtmlEncode($Page->DIAGNOSA_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->NO_SURAT_KET->getDisplayValue($Page->NO_SURAT_KET->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="V_RAWAT_INAP" data-field="x_NO_SURAT_KET" data-hidden="1" name="x_NO_SURAT_KET" id="x_NO_SURAT_KET" value="<?= HtmlEncode($Page->NO_SURAT_KET->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
?>
<?php
    if (in_array("TREATMENT_BAYAR", explode(",", $Page->getCurrentDetailTable())) && $TREATMENT_BAYAR->DetailEdit) {
        if ($firstActiveDetailTable == "" || $firstActiveDetailTable == "TREATMENT_BAYAR") {
            $firstActiveDetailTable = "TREATMENT_BAYAR";
        }
?>
        <li class="nav-item"><a class="nav-link <?= $Page->DetailPages->pageStyle("TREATMENT_BAYAR") ?>" href="#tab_TREATMENT_BAYAR" data-toggle="tab"><?= $Language->tablePhrase("TREATMENT_BAYAR", "TblCaption") ?></a></li>
<?php
    }
?>
    </ul><!-- /.nav -->
...
?>
        <div class="tab-pane <?= $Page->DetailPages->pageStyle("TREATMENT_BAYAR") ?>" id="tab_TREATMENT_BAYAR"><!-- page* -->
<?php include_once "TreatmentBayarGrid.php" ?>
        </div><!-- /page* -->
<?php } ?>
    </div><!-- /.tab-content -->
</div><!-- /tabs -->
