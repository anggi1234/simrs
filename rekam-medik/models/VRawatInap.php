...
class VRawatInap extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // VISIT_ID
        $this->VISIT_ID = new DbField('V_RAWAT_INAP', 'V_RAWAT_INAP', 'x_VISIT_ID', 'VISIT_ID', '[VISIT_ID]', '[VISIT_ID]', 200, 50, -1, false, '[VISIT_ID]', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        $this->THEID->IsForeignKey = true; // Foreign key field
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "PASIEN_DIAGNOSA") {
            ...
            $detailUrl .= "&" . GetForeignKeyUrl("fk_THENAME", $this->THENAME->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_THEID", $this->THEID->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "TREATMENT_BAYAR") {
            $detailUrl = Container("TREATMENT_BAYAR")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "VRawatInapList";
        }
        return $detailUrl;
    }
    ...
    {
        // Cascade Update detail table 'PASIEN_DIAGNOSA'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($rsold && (isset($rs['THENAME']) && $rsold['THENAME'] != $rs['THENAME'])) { // Update detail field 'THENAME'
            $cascadeUpdate = true;
            $rscascade['THENAME'] = $rs['THENAME'];
        }
        if ($rsold && (isset($rs['THEID']) && $rsold['THEID'] != $rs['THEID'])) { // Update detail field 'THEID'
            $cascadeUpdate = true;
            $rscascade['THEID'] = $rs['THEID'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("PASIEN_DIAGNOSA")->loadRs("[NO_REGISTRATION] = " . QuotedValue($rsold['NO_REGISTRATION'], DATATYPE_STRING, 'DB') . " AND " . "[VISIT_ID] = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB') . " AND " . "[THENAME] = " . QuotedValue($rsold['THENAME'], DATATYPE_STRING, 'DB') . " AND " . "[THEID] = " . QuotedValue($rsold['THEID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("PASIEN_DIAGNOSA")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("PASIEN_DIAGNOSA")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("PASIEN_DIAGNOSA")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'TREATMENT_BAYAR'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['VISIT_ID']) && $rsold['VISIT_ID'] != $rs['VISIT_ID'])) { // Update detail field 'VISIT_ID'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("TREATMENT_BAYAR")->loadRs("[VISIT_ID] = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'ID';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("TREATMENT_BAYAR")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("TREATMENT_BAYAR")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("TREATMENT_BAYAR")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'PASIEN_DIAGNOSA'
        $dtlrows = Container("PASIEN_DIAGNOSA")->loadRs("[NO_REGISTRATION] = " . QuotedValue($rs['NO_REGISTRATION'], DATATYPE_STRING, "DB") . " AND " . "[VISIT_ID] = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB") . " AND " . "[THENAME] = " . QuotedValue($rs['THENAME'], DATATYPE_STRING, "DB") . " AND " . "[THEID] = " . QuotedValue($rs['THEID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("PASIEN_DIAGNOSA")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("PASIEN_DIAGNOSA")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("PASIEN_DIAGNOSA")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'TREATMENT_BAYAR'
        $dtlrows = Container("TREATMENT_BAYAR")->loadRs("[VISIT_ID] = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("TREATMENT_BAYAR")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("TREATMENT_BAYAR")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("TREATMENT_BAYAR")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("VRawatInapEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("VRawatInapAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->CLASS_ID->EditValue = FormatNumber($this->CLASS_ID->EditValue, 0, -2, -2, -2);
        $this->CLASS_ID->ViewCustomAttributes = "";
        ...
        $this->MEASURE_ID->EditValue = FormatNumber($this->MEASURE_ID->EditValue, 0, -2, -2, -2);
        $this->MEASURE_ID->ViewCustomAttributes = "";
        ...
        $curVal = trim(strval($this->BED_ID->CurrentValue));
        if ($curVal != "") {
            $this->BED_ID->EditValue = $this->BED_ID->lookupCacheOption($curVal);
            if ($this->BED_ID->EditValue === null) { // Lookup from database
                $filterWrk = "[BED_ID]" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                $sqlWrk = $this->BED_ID->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->BED_ID->Lookup->renderViewRow($rswrk[0]);
                    $this->BED_ID->EditValue = $this->BED_ID->displayValue($arwrk);
                } else {
                    $this->BED_ID->EditValue = $this->BED_ID->CurrentValue;
                }
            }
        } else {
            $this->BED_ID->EditValue = null;
        }
        $this->BED_ID->ViewCustomAttributes = "";
        ...
        $this->STATUS_PASIEN_ID->EditValue = FormatNumber($this->STATUS_PASIEN_ID->EditValue, 0, -2, -2, -2);
        $this->STATUS_PASIEN_ID->ViewCustomAttributes = "";
        ...
        $this->MODIFIED_DATE->EditValue = $this->MODIFIED_DATE->CurrentValue;
        $this->MODIFIED_DATE->EditValue = FormatDateTime($this->MODIFIED_DATE->EditValue, 0);
        $this->MODIFIED_DATE->ViewCustomAttributes = "";
        ...
        $this->PRINTQ->EditValue = FormatNumber($this->PRINTQ->EditValue, 0, -2, -2, -2);
        $this->PRINTQ->ViewCustomAttributes = "";
        ...
        $this->STATUS_TARIF->EditValue = FormatNumber($this->STATUS_TARIF->EditValue, 0, -2, -2, -2);
        $this->STATUS_TARIF->ViewCustomAttributes = "";
        ...
        $this->CLINIC_TYPE->EditValue = FormatNumber($this->CLINIC_TYPE->EditValue, 0, -2, -2, -2);
        $this->CLINIC_TYPE->ViewCustomAttributes = "";
        ...
        $this->THEORDER->EditValue = FormatNumber($this->THEORDER->EditValue, 0, -2, -2, -2);
        $this->THEORDER->ViewCustomAttributes = "";
        ...
        $this->modified_datesys->EditValue = $this->modified_datesys->CurrentValue;
        $this->modified_datesys->EditValue = FormatDateTime($this->modified_datesys->EditValue, 0);
        $this->modified_datesys->ViewCustomAttributes = "";
        ...
        $this->SPPBILLDATE->EditValue = $this->SPPBILLDATE->CurrentValue;
        $this->SPPBILLDATE->EditValue = FormatDateTime($this->SPPBILLDATE->EditValue, 0);
        $this->SPPBILLDATE->ViewCustomAttributes = "";
        ...
        $this->SPPKASIRDATE->EditValue = $this->SPPKASIRDATE->CurrentValue;
        $this->SPPKASIRDATE->EditValue = FormatDateTime($this->SPPKASIRDATE->EditValue, 0);
        $this->SPPKASIRDATE->ViewCustomAttributes = "";
        ...
        $this->SPPPOLIDATE->EditValue = $this->SPPPOLIDATE->CurrentValue;
        $this->SPPPOLIDATE->EditValue = FormatDateTime($this->SPPPOLIDATE->EditValue, 0);
        $this->SPPPOLIDATE->ViewCustomAttributes = "";
