...
class VRawatInapList extends VRawatInap
{
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_TREATMENT_BAYAR");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'TREATMENT_BAYAR') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'PASIEN_DIAGNOSA')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("PasienDiagnosaList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_THENAME", $this->THENAME->CurrentValue) . "&" . GetForeignKeyUrl("fk_THEID", $this->THEID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        // "detail_TREATMENT_BAYAR"
        $opt = $this->ListOptions["detail_TREATMENT_BAYAR"];
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_BAYAR')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("TREATMENT_BAYAR", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentBayarList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("TreatmentBayarGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_RAWAT_INAP')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_BAYAR");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_RAWAT_INAP')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_BAYAR");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
