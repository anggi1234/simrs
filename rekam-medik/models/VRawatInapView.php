...
class VRawatInapView extends VRawatInap
{
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PasienDiagnosaList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_THENAME", $this->THENAME->CurrentValue) . "&" . GetForeignKeyUrl("fk_THEID", $this->THEID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        // "detail_TREATMENT_BAYAR"
        $item = &$option->add("detail_TREATMENT_BAYAR");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("TREATMENT_BAYAR", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("TreatmentBayarList?" . Config("TABLE_SHOW_MASTER") . "=V_RAWAT_INAP&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("TreatmentBayarGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_RAWAT_INAP')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_BAYAR"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "TREATMENT_BAYAR";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_RAWAT_INAP')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_BAYAR"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "TREATMENT_BAYAR";
        }
        if ($links != "") {
            $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-toggle=\"dropdown\"></button>";
            $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        $body = "<div class=\"btn-group btn-group-sm ew-btn-group\">" . $body . "</div>";
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'TREATMENT_BAYAR');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "TREATMENT_BAYAR";
        }
        if ($this->ShowMultipleDetails) {
            $item->Visible = false;
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("PASIEN_DIAGNOSA", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->THEID->IsDetailKey = true;
                    $detailPageObj->THEID->CurrentValue = $this->THEID->CurrentValue;
                    $detailPageObj->THEID->setSessionValue($detailPageObj->THEID->CurrentValue);
                    $detailPageObj->THEADDRESS->setSessionValue(""); // Clear session key
                }
            }
            if (in_array("TREATMENT_BAYAR", $detailTblVar)) {
                $detailPageObj = Container("TreatmentBayarGrid");
                if ($detailPageObj->DetailView) {
                    $detailPageObj->CurrentMode = "view";
