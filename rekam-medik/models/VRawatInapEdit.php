...
class VRawatInapEdit extends VRawatInap
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("TREATMENT_BAYAR", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        ...
        if (!$rsold) {
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    $detailPage = Container("TreatmentBayarGrid");
                    if (in_array("TREATMENT_BAYAR", $detailTblVar) && $detailPage->DetailEdit) {
                        $Security->loadCurrentUserLevel($this->ProjectID . "TREATMENT_BAYAR"); // Load user level of detail table
                        $editRow = $detailPage->gridUpdate();
                        $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                    }
                }
