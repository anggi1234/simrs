...
<input type="hidden" data-table="PASIEN" data-field="x_NO_REGISTRATION" data-hidden="1" name="x_NO_REGISTRATION" id="x_NO_REGISTRATION" value="<?= HtmlEncode($Page->NO_REGISTRATION->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->newapp->Visible) { // newapp ?>
    <div id="r_newapp" class="form-group row">
        <label id="elh_PASIEN_newapp" for="x_newapp" class="<?= $Page->LeftColumnClass ?>"><?= $Page->newapp->caption() ?><?= $Page->newapp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->newapp->cellAttributes() ?>>
<span id="el_PASIEN_newapp">
<input type="<?= $Page->newapp->getInputTextType() ?>" data-table="PASIEN" data-field="x_newapp" name="x_newapp" id="x_newapp" size="30" maxlength="1" placeholder="<?= HtmlEncode($Page->newapp->getPlaceHolder()) ?>" value="<?= $Page->newapp->EditValue ?>"<?= $Page->newapp->editAttributes() ?> aria-describedby="x_newapp_help">
