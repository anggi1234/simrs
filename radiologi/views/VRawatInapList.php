...
loadjs.ready("head", function () {
    ...
    fV_RAWAT_INAPlistsrch.addFields([
    ...
    // Set invalid fields
    $(function() {
        fV_RAWAT_INAPlistsrch.setInvalid();
    });

    // Validate form
    fV_RAWAT_INAPlistsrch.validate = function () {
    ...
    // Form_CustomValidate
    fV_RAWAT_INAPlistsrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fV_RAWAT_INAPlistsrch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fV_RAWAT_INAPlistsrch.lists.CLINIC_ID = <?= $Page->CLINIC_ID->toClientList($Page) ?>;
...
<input type="hidden" name="z_NO_REGISTRATION" id="z_NO_REGISTRATION" value="LIKE">
        ...
        <span id="el_V_RAWAT_INAP_NO_REGISTRATION" class="ew-search-field">
<input type="<?= $Page->NO_REGISTRATION->getInputTextType() ?>" data-table="V_RAWAT_INAP" data-field="x_NO_REGISTRATION" name="x_NO_REGISTRATION" id="x_NO_REGISTRATION" size="30" maxlength="25" placeholder="<?= HtmlEncode($Page->NO_REGISTRATION->getPlaceHolder()) ?>" value="<?= $Page->NO_REGISTRATION->EditValue ?>"<?= $Page->NO_REGISTRATION->editAttributes() ?>>
...
<div id="xsr_<?= $Page->SearchRowCount ?>" class="ew-row d-sm-flex">
    <?php
     ...
     ?>
    <div id="xsc_THENAME" class="ew-cell form-group">
        <label for="x_THENAME" class="ew-search-caption ew-label"><?= $Page->THENAME->caption() ?></label>
        <span class="ew-search-operator">
<?= $Language->phrase("LIKE") ?>
<input type="hidden" name="z_THENAME" id="z_THENAME" value="LIKE">
</span>
        <span id="el_V_RAWAT_INAP_THENAME" class="ew-search-field">
<input type="<?= $Page->THENAME->getInputTextType() ?>" data-table="V_RAWAT_INAP" data-field="x_THENAME" name="x_THENAME" id="x_THENAME" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->THENAME->getPlaceHolder()) ?>" value="<?= $Page->THENAME->EditValue ?>"<?= $Page->THENAME->editAttributes() ?>>
...
<input type="hidden" name="z_CLINIC_ID" id="z_CLINIC_ID" value="LIKE">
        ...
        <span id="el_V_RAWAT_INAP_CLINIC_ID" class="ew-search-field">
    <select
        id="x_CLINIC_ID"
        name="x_CLINIC_ID"
...
<?php if ($Page->TREATMENT->Visible) { // TREATMENT ?>
        <th data-name="TREATMENT" class="<?= $Page->TREATMENT->headerCellClass() ?>"><div id="elh_V_RAWAT_INAP_TREATMENT" class="V_RAWAT_INAP_TREATMENT"><?= $Page->renderSort($Page->TREATMENT) ?></div></th>
<?php } ?>
...
<?php if ($Page->DESCRIPTION->Visible) { // DESCRIPTION ?>
        <th data-name="DESCRIPTION" class="<?= $Page->DESCRIPTION->headerCellClass() ?>"><div id="elh_V_RAWAT_INAP_DESCRIPTION" class="V_RAWAT_INAP_DESCRIPTION"><?= $Page->renderSort($Page->DESCRIPTION) ?></div></th>
<?php } ?>
...
<?php if ($Page->BED_ID->Visible) { // BED_ID ?>
        <th data-name="BED_ID" class="<?= $Page->BED_ID->headerCellClass() ?>"><div id="elh_V_RAWAT_INAP_BED_ID" class="V_RAWAT_INAP_BED_ID"><?= $Page->renderSort($Page->BED_ID) ?></div></th>
<?php } ?>
