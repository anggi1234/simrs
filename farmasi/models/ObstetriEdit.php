...
class ObstetriEdit extends Obstetri
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("OBSTETRI_ID") ?? Key(0) ?? Route(2)) !== null) {
                $this->OBSTETRI_ID->setQueryStringValue($keyValue);
                $this->OBSTETRI_ID->setOldValue($this->OBSTETRI_ID->QueryStringValue);
            } elseif (Post("OBSTETRI_ID") !== null) {
                $this->OBSTETRI_ID->setFormValue(Post("OBSTETRI_ID"));
                $this->OBSTETRI_ID->setOldValue($this->OBSTETRI_ID->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("OBSTETRI_ID") ?? Route("OBSTETRI_ID")) !== null) {
                    $this->OBSTETRI_ID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
