 ...
 * Page class
 */
class VTreatList extends VTreat
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VTreatList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_TREATlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "VTreatDelete";
        $this->MultiUpdateUrl = "VTreatUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fV_TREATlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->SERVICE_TIME->AdvancedSearch->toJson(), ","); // Field SERVICE_TIME
        $filterList = Concat($filterList, $this->TAKEN_TIME->AdvancedSearch->toJson(), ","); // Field TAKEN_TIME
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fV_TREATlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field SERVICE_TIME
        $this->SERVICE_TIME->AdvancedSearch->SearchValue = @$filter["x_SERVICE_TIME"];
        $this->SERVICE_TIME->AdvancedSearch->SearchOperator = @$filter["z_SERVICE_TIME"];
        $this->SERVICE_TIME->AdvancedSearch->SearchCondition = @$filter["v_SERVICE_TIME"];
        $this->SERVICE_TIME->AdvancedSearch->SearchValue2 = @$filter["y_SERVICE_TIME"];
        $this->SERVICE_TIME->AdvancedSearch->SearchOperator2 = @$filter["w_SERVICE_TIME"];
        $this->SERVICE_TIME->AdvancedSearch->save();

        // Field TAKEN_TIME
        $this->TAKEN_TIME->AdvancedSearch->SearchValue = @$filter["x_TAKEN_TIME"];
        $this->TAKEN_TIME->AdvancedSearch->SearchOperator = @$filter["z_TAKEN_TIME"];
        $this->TAKEN_TIME->AdvancedSearch->SearchCondition = @$filter["v_TAKEN_TIME"];
        $this->TAKEN_TIME->AdvancedSearch->SearchValue2 = @$filter["y_TAKEN_TIME"];
        $this->TAKEN_TIME->AdvancedSearch->SearchOperator2 = @$filter["w_TAKEN_TIME"];
        $this->TAKEN_TIME->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->NOSEP, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->SERVICE_TIME); // SERVICE_TIME
            $this->updateSort($this->TAKEN_TIME); // TAKEN_TIME
            $this->updateSort($this->modified_datesys); // modified_datesys
            $this->updateSort($this->TRANS_ID); // TRANS_ID
            $this->updateSort($this->SPPBILL); // SPPBILL
            $this->updateSort($this->SPPBILLDATE); // SPPBILLDATE
            $this->updateSort($this->SPPBILLUSER); // SPPBILLUSER
            $this->updateSort($this->SPPKASIR); // SPPKASIR
            $this->updateSort($this->SPPKASIRDATE); // SPPKASIRDATE
            $this->updateSort($this->SPPKASIRUSER); // SPPKASIRUSER
            $this->updateSort($this->SPPPOLI); // SPPPOLI
            $this->updateSort($this->SPPPOLIUSER); // SPPPOLIUSER
            $this->updateSort($this->SPPPOLIDATE); // SPPPOLIDATE
            $this->updateSort($this->NOSEP); // NOSEP
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fV_TREATlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fV_TREATlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fV_TREATlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fV_TREATlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
