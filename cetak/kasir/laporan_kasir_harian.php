...
$sqlitem = "SELECT
    ...
    REPLACE(c.userlevelname,
        'POLIKLINIK',
        '') AS userlevelname,
    e.namadokter,
    d.nama AS carabayar,
    IFNULL(biaya_pendaftaran, 0) + IFNULL(biaya_jasar, 0) + IFNULL(biaya_penj_non_medis, 0) + IFNULL(biaya_asuhan_keperawatan, 0) + IFNULL(biaya_pemeriksaan, 0) AS karcis,
    IFNULL(a.biaya_tind_tmno, 0) AS biaya_tind_tmno,
    IFNULL(a.biaya_tind_tmo, 0) AS biaya_tind_tmo,
    IFNULL(a.biaya_tind_keperawatan, 0) AS biaya_tind_keperawatan,
    IFNULL(a.biaya_tind_persalinan, 0) AS biaya_tind_persalinan,
    IFNULL(a.biaya_penj_laboratorium, 0) AS biaya_penj_laboratorium,
    IFNULL(a.biaya_penj_radiologi, 0) AS biaya_penj_radiologi,
    IFNULL(a.biaya_penj_fisioterapi, 0) AS biaya_penj_fisioterapi,
    IFNULL(a.biaya_obat, 0) - IFNULL(a.biaya_obat_retur, 0) AS biaya_obat,
    IFNULL(a.biaya_ekg, 0) AS biaya_ekg,
    IFNULL(a.biaya_usg, 0) AS biaya_usg,
	IFNULL(biaya_bhp_tmno, 0) + IFNULL(biaya_bhp_tmo, 0) + IFNULL(biaya_bhp_keperawatan, 0) + IFNULL(biaya_bhp_persalinan, 0) + IFNULL(biaya_bhp_oksigen, 0) + IFNULL(biaya_bhp_ekg, 0) + IFNULL(biaya_bhp_usg, 0) AS bhp,
    a.total_keseluruhan,
    (SELECT
        ...
        FROM
            bill_detail_penyakit z
                LEFT JOIN
            icd_eklaim y ON z.icd10 = y.code
        WHERE
            z.idxdaftar = a.idxdaftar
        GROUP BY z.idxdaftar) AS diagnosa,
    (SELECT
            CONCAT(y.keterangan, ' (', ifnull(x.userlevelname,''), ')') AS konsul
        FROM
            bill_detail_transfer_pasien z
                LEFT JOIN
            m_statuskeluar y ON z.id_status_keluar = y.status
                LEFT JOIN
            userlevels x ON z.userlevelid_transfer = x.userlevelid
        WHERE
            z.idxdaftar = a.idxdaftar
        GROUP BY z.idxdaftar) AS konsul,
a.biaya_pel_ambulan,
a.biaya_tind_jenazah,
a.biaya_bimbingan_rohani,
a.biaya_akomodasi,
a.biaya_makan
FROM
    data a
...
WHERE a.tglreg >= '$dari_tanggal' and a.tglreg <= '$sampai_tanggal' and d.payor_id=$kdcarabayar AND c.id_jenis_pelayanan_kasir=$id_jenis_pelayanan_kasir";
...
<table width="100%" border="1" class="tabel">
  <tbody>
    <tr>
      ...
      <td width="3%" align="center">No Pel</td>
