...
<?php if ($Grid->AMOUNT->Visible) { // AMOUNT ?>
        <th data-name="AMOUNT" class="<?= $Grid->AMOUNT->headerCellClass() ?>" style="white-space: nowrap;"><div id="elh_TREATMENT_BILL_AMOUNT" class="TREATMENT_BILL_AMOUNT"><?= $Grid->renderSort($Grid->AMOUNT) ?></div></th>
<?php } ?>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Grid->DOCTOR->getDisplayValue($Grid->DOCTOR->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="TREATMENT_BILL" data-field="x_DOCTOR" data-hidden="1" name="x<?= $Grid->RowIndex ?>_DOCTOR" id="x<?= $Grid->RowIndex ?>_DOCTOR" value="<?= HtmlEncode($Grid->DOCTOR->CurrentValue) ?>">
<?php } ?>
<?php if ($Grid->RowType == ROWTYPE_VIEW) { // View record ?>
