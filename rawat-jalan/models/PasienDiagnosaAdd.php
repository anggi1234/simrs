...
class PasienDiagnosaAdd extends PasienDiagnosa
{
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->REPORT_DATE->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->REPORT_DATE->setFormValue($val);
            }
            $this->REPORT_DATE->CurrentValue = UnFormatDateTime($this->REPORT_DATE->CurrentValue, 0);
        }
        ...
        // Check field name 'DIAGNOSA_DESC' first before field var 'x_DIAGNOSA_DESC'
        $val = $CurrentForm->hasValue("DIAGNOSA_DESC") ? $CurrentForm->getValue("DIAGNOSA_DESC") : $CurrentForm->getValue("x_DIAGNOSA_DESC");
        if (!$this->DIAGNOSA_DESC->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->DIAGNOSA_DESC->Visible = false; // Disable update for API request
            } else {
                $this->DIAGNOSA_DESC->setFormValue($val);
            }
        }
        ...
        // Check field name 'DIAGNOSA_DESC_02' first before field var 'x_DIAGNOSA_DESC_02'
        $val = $CurrentForm->hasValue("DIAGNOSA_DESC_02") ? $CurrentForm->getValue("DIAGNOSA_DESC_02") : $CurrentForm->getValue("x_DIAGNOSA_DESC_02");
        if (!$this->DIAGNOSA_DESC_02->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->DIAGNOSA_DESC_02->Visible = false; // Disable update for API request
            } else {
                $this->DIAGNOSA_DESC_02->setFormValue($val);
            }
        }

        // Check field name 'DIAGNOSA_DESC_03' first before field var 'x_DIAGNOSA_DESC_03'
        $val = $CurrentForm->hasValue("DIAGNOSA_DESC_03") ? $CurrentForm->getValue("DIAGNOSA_DESC_03") : $CurrentForm->getValue("x_DIAGNOSA_DESC_03");
        if (!$this->DIAGNOSA_DESC_03->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->DIAGNOSA_DESC_03->Visible = false; // Disable update for API request
            } else {
                $this->DIAGNOSA_DESC_03->setFormValue($val);
            }
        }

        // Check field name 'DIAGNOSA_DESC_04' first before field var 'x_DIAGNOSA_DESC_04'
        $val = $CurrentForm->hasValue("DIAGNOSA_DESC_04") ? $CurrentForm->getValue("DIAGNOSA_DESC_04") : $CurrentForm->getValue("x_DIAGNOSA_DESC_04");
        if (!$this->DIAGNOSA_DESC_04->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->DIAGNOSA_DESC_04->Visible = false; // Disable update for API request
            } else {
                $this->DIAGNOSA_DESC_04->setFormValue($val);
            }
        }

        // Check field name 'DIAGNOSA_DESC_05' first before field var 'x_DIAGNOSA_DESC_05'
        $val = $CurrentForm->hasValue("DIAGNOSA_DESC_05") ? $CurrentForm->getValue("DIAGNOSA_DESC_05") : $CurrentForm->getValue("x_DIAGNOSA_DESC_05");
        if (!$this->DIAGNOSA_DESC_05->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->DIAGNOSA_DESC_05->Visible = false; // Disable update for API request
            } else {
                $this->DIAGNOSA_DESC_05->setFormValue($val);
            }
        }

        // Check field name 'PROCEDURE_DESC_02' first before field var 'x_PROCEDURE_DESC_02'
        $val = $CurrentForm->hasValue("PROCEDURE_DESC_02") ? $CurrentForm->getValue("PROCEDURE_DESC_02") : $CurrentForm->getValue("x_PROCEDURE_DESC_02");
        if (!$this->PROCEDURE_DESC_02->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->PROCEDURE_DESC_02->Visible = false; // Disable update for API request
            } else {
                $this->PROCEDURE_DESC_02->setFormValue($val);
            }
        }

        // Check field name 'PROCEDURE_DESC_03' first before field var 'x_PROCEDURE_DESC_03'
        $val = $CurrentForm->hasValue("PROCEDURE_DESC_03") ? $CurrentForm->getValue("PROCEDURE_DESC_03") : $CurrentForm->getValue("x_PROCEDURE_DESC_03");
        if (!$this->PROCEDURE_DESC_03->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->PROCEDURE_DESC_03->Visible = false; // Disable update for API request
            } else {
                $this->PROCEDURE_DESC_03->setFormValue($val);
            }
        }

        // Check field name 'PROCEDURE_DESC_04' first before field var 'x_PROCEDURE_DESC_04'
        $val = $CurrentForm->hasValue("PROCEDURE_DESC_04") ? $CurrentForm->getValue("PROCEDURE_DESC_04") : $CurrentForm->getValue("x_PROCEDURE_DESC_04");
        if (!$this->PROCEDURE_DESC_04->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->PROCEDURE_DESC_04->Visible = false; // Disable update for API request
            } else {
                $this->PROCEDURE_DESC_04->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->DIAGNOSA_DESC->CurrentValue = $this->DIAGNOSA_DESC->FormValue;
        ...
        $this->TGLKONTROL->CurrentValue = UnFormatDateTime($this->TGLKONTROL->CurrentValue, 0);
        $this->DIAGNOSA_DESC_02->CurrentValue = $this->DIAGNOSA_DESC_02->FormValue;
        $this->DIAGNOSA_DESC_03->CurrentValue = $this->DIAGNOSA_DESC_03->FormValue;
        $this->DIAGNOSA_DESC_04->CurrentValue = $this->DIAGNOSA_DESC_04->FormValue;
        $this->DIAGNOSA_DESC_05->CurrentValue = $this->DIAGNOSA_DESC_05->FormValue;
        $this->PROCEDURE_DESC_02->CurrentValue = $this->PROCEDURE_DESC_02->FormValue;
        $this->PROCEDURE_DESC_03->CurrentValue = $this->PROCEDURE_DESC_03->FormValue;
        $this->PROCEDURE_DESC_04->CurrentValue = $this->PROCEDURE_DESC_04->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->DIAGNOSA_DESC->EditValue = HtmlEncode($this->DIAGNOSA_DESC->CurrentValue);
            ...
            $this->DIAGNOSA_DESC_02->EditValue = HtmlEncode($this->DIAGNOSA_DESC_02->CurrentValue);
            ...
            $this->DIAGNOSA_DESC_03->EditValue = HtmlEncode($this->DIAGNOSA_DESC_03->CurrentValue);
            ...
            $this->DIAGNOSA_DESC_04->EditValue = HtmlEncode($this->DIAGNOSA_DESC_04->CurrentValue);
            ...
            $this->DIAGNOSA_DESC_05->EditValue = HtmlEncode($this->DIAGNOSA_DESC_05->CurrentValue);
            ...
            $this->PROCEDURE_DESC_02->EditValue = HtmlEncode($this->PROCEDURE_DESC_02->CurrentValue);
            ...
            $this->PROCEDURE_DESC_03->EditValue = HtmlEncode($this->PROCEDURE_DESC_03->CurrentValue);
            ...
            $this->PROCEDURE_DESC_04->EditValue = HtmlEncode($this->PROCEDURE_DESC_04->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->DIAGNOSA_DESC->Required) {
            if (!$this->DIAGNOSA_DESC->IsDetailKey && EmptyValue($this->DIAGNOSA_DESC->FormValue)) {
                $this->DIAGNOSA_DESC->addErrorMessage(str_replace("%s", $this->DIAGNOSA_DESC->caption(), $this->DIAGNOSA_DESC->RequiredErrorMessage));
            }
        }
        ...
        if ($this->DIAGNOSA_DESC_02->Required) {
            if (!$this->DIAGNOSA_DESC_02->IsDetailKey && EmptyValue($this->DIAGNOSA_DESC_02->FormValue)) {
                $this->DIAGNOSA_DESC_02->addErrorMessage(str_replace("%s", $this->DIAGNOSA_DESC_02->caption(), $this->DIAGNOSA_DESC_02->RequiredErrorMessage));
            }
        }
        if ($this->DIAGNOSA_DESC_03->Required) {
            if (!$this->DIAGNOSA_DESC_03->IsDetailKey && EmptyValue($this->DIAGNOSA_DESC_03->FormValue)) {
                $this->DIAGNOSA_DESC_03->addErrorMessage(str_replace("%s", $this->DIAGNOSA_DESC_03->caption(), $this->DIAGNOSA_DESC_03->RequiredErrorMessage));
            }
        }
        if ($this->DIAGNOSA_DESC_04->Required) {
            if (!$this->DIAGNOSA_DESC_04->IsDetailKey && EmptyValue($this->DIAGNOSA_DESC_04->FormValue)) {
                $this->DIAGNOSA_DESC_04->addErrorMessage(str_replace("%s", $this->DIAGNOSA_DESC_04->caption(), $this->DIAGNOSA_DESC_04->RequiredErrorMessage));
            }
        }
        if ($this->DIAGNOSA_DESC_05->Required) {
            if (!$this->DIAGNOSA_DESC_05->IsDetailKey && EmptyValue($this->DIAGNOSA_DESC_05->FormValue)) {
                $this->DIAGNOSA_DESC_05->addErrorMessage(str_replace("%s", $this->DIAGNOSA_DESC_05->caption(), $this->DIAGNOSA_DESC_05->RequiredErrorMessage));
            }
        }
        if ($this->PROCEDURE_DESC_02->Required) {
            if (!$this->PROCEDURE_DESC_02->IsDetailKey && EmptyValue($this->PROCEDURE_DESC_02->FormValue)) {
                $this->PROCEDURE_DESC_02->addErrorMessage(str_replace("%s", $this->PROCEDURE_DESC_02->caption(), $this->PROCEDURE_DESC_02->RequiredErrorMessage));
            }
        }
        if ($this->PROCEDURE_DESC_03->Required) {
            if (!$this->PROCEDURE_DESC_03->IsDetailKey && EmptyValue($this->PROCEDURE_DESC_03->FormValue)) {
                $this->PROCEDURE_DESC_03->addErrorMessage(str_replace("%s", $this->PROCEDURE_DESC_03->caption(), $this->PROCEDURE_DESC_03->RequiredErrorMessage));
            }
        }
        if ($this->PROCEDURE_DESC_04->Required) {
            if (!$this->PROCEDURE_DESC_04->IsDetailKey && EmptyValue($this->PROCEDURE_DESC_04->FormValue)) {
                $this->PROCEDURE_DESC_04->addErrorMessage(str_replace("%s", $this->PROCEDURE_DESC_04->caption(), $this->PROCEDURE_DESC_04->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // REPORT_DATE
        $this->REPORT_DATE->CurrentValue = CurrentDateTime();
        $this->REPORT_DATE->setDbValueDef($rsnew, $this->REPORT_DATE->CurrentValue, null);

        // DIAGNOSA_ID
        $this->DIAGNOSA_ID->setDbValueDef($rsnew, $this->DIAGNOSA_ID->CurrentValue, null, false);

        // DIAGNOSA_DESC
        $this->DIAGNOSA_DESC->setDbValueDef($rsnew, $this->DIAGNOSA_DESC->CurrentValue, null, false);
        ...
        // INSTRUCTION
        $this->INSTRUCTION->setDbValueDef($rsnew, $this->INSTRUCTION->CurrentValue, null, false);
        ...
        // DIAGNOSA_DESC_02
        $this->DIAGNOSA_DESC_02->setDbValueDef($rsnew, $this->DIAGNOSA_DESC_02->CurrentValue, null, false);

        // DIAGNOSA_DESC_03
        $this->DIAGNOSA_DESC_03->setDbValueDef($rsnew, $this->DIAGNOSA_DESC_03->CurrentValue, null, false);

        // DIAGNOSA_DESC_04
        $this->DIAGNOSA_DESC_04->setDbValueDef($rsnew, $this->DIAGNOSA_DESC_04->CurrentValue, null, false);

        // DIAGNOSA_DESC_05
        $this->DIAGNOSA_DESC_05->setDbValueDef($rsnew, $this->DIAGNOSA_DESC_05->CurrentValue, null, false);

        // PROCEDURE_DESC_02
        $this->PROCEDURE_DESC_02->setDbValueDef($rsnew, $this->PROCEDURE_DESC_02->CurrentValue, null, false);

        // PROCEDURE_DESC_03
        $this->PROCEDURE_DESC_03->setDbValueDef($rsnew, $this->PROCEDURE_DESC_03->CurrentValue, null, false);

        // PROCEDURE_DESC_04
        $this->PROCEDURE_DESC_04->setDbValueDef($rsnew, $this->PROCEDURE_DESC_04->CurrentValue, null, false);
