...
class TreatmentBill extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // IDXDAFTAR
        $this->IDXDAFTAR = new DbField('TREATMENT_BILL', 'TREATMENT_BILL', 'x_IDXDAFTAR', 'IDXDAFTAR', '[IDXDAFTAR]', 'CAST([IDXDAFTAR] AS NVARCHAR)', 3, 4, -1, false, '[IDXDAFTAR]', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->IDXDAFTAR->IsAutoIncrement = true; // Autoincrement field
        $this->IDXDAFTAR->IsPrimaryKey = true; // Primary key field
        $this->IDXDAFTAR->IsForeignKey = true; // Foreign key field
    ...
    public function getMasterFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "PASIEN_VISITATION") {
            ...
            if ($this->IDXDAFTAR->getSessionValue() != "") {
                $masterFilter .= " AND " . GetForeignKeySql("[IDXDAFTAR]", $this->IDXDAFTAR->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        ...
        if ($this->getCurrentMasterTable() == "V_KUNJUNGAN_PASIEN") {
            if ($this->NO_REGISTRATION->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("dbo.PASIEN_VISITATION.NO_REGISTRATION", $this->NO_REGISTRATION->getSessionValue(), DATATYPE_STRING, "DB");
            } else {
                return "";
            }
            if ($this->IDXDAFTAR->getSessionValue() != "") {
                $masterFilter .= " AND " . GetForeignKeySql("dbo.PASIEN_VISITATION.IDXDAFTAR", $this->IDXDAFTAR->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
            if ($this->VISIT_ID->getSessionValue() != "") {
                $masterFilter .= " AND " . GetForeignKeySql("dbo.PASIEN_VISITATION.VISIT_ID", $this->VISIT_ID->getSessionValue(), DATATYPE_STRING, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "PASIEN_VISITATION") {
            ...
            if ($this->IDXDAFTAR->getSessionValue() != "") {
                $detailFilter .= " AND " . GetForeignKeySql("[IDXDAFTAR]", $this->IDXDAFTAR->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
            if ($this->THENAME->getSessionValue() != "") {
    ...
    {
        return "[VISIT_ID]='@VISIT_ID@' AND [NO_REGISTRATION]='@NO_REGISTRATION@' AND [IDXDAFTAR]=@IDXDAFTAR@ AND [DIANTAR_OLEH]='@DIANTAR_OLEH@' AND [VISITOR_ADDRESS]='@VISITOR_ADDRESS@'";
    }
    // Detail filter
    public function sqlDetailFilter_PASIEN_VISITATION()
    {
        return "[VISIT_ID]='@VISIT_ID@' AND [NO_REGISTRATION]='@NO_REGISTRATION@' AND [IDXDAFTAR]=@IDXDAFTAR@ AND [THENAME]='@THENAME@' AND [THEADDRESS]='@THEADDRESS@'";
    }

    // Master filter
    public function sqlMasterFilter_V_KUNJUNGAN_PASIEN()
    {
        return "dbo.PASIEN_VISITATION.NO_REGISTRATION='@NO_REGISTRATION@' AND dbo.PASIEN_VISITATION.IDXDAFTAR=@IDXDAFTAR@ AND dbo.PASIEN_VISITATION.VISIT_ID='@VISIT_ID@'";
    }
    // Detail filter
    public function sqlDetailFilter_V_KUNJUNGAN_PASIEN()
    {
        return "[NO_REGISTRATION]='@NO_REGISTRATION@' AND [IDXDAFTAR]=@IDXDAFTAR@ AND [VISIT_ID]='@VISIT_ID@'";
    }
    ...
    public function addMasterUrl($url)
    {
        ...
        if ($this->getCurrentMasterTable() == "PASIEN_VISITATION" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            ...
            $url .= "&" . GetForeignKeyUrl("fk_IDXDAFTAR", $this->IDXDAFTAR->CurrentValue ?? $this->IDXDAFTAR->getSessionValue());
