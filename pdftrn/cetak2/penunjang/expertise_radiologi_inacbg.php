...
$sqlisi="SELECT 
    a.userlevelid, a.tanggal, a.klinis, a.bacaan, a.kesan
FROM
...
$sqlidentitas="SELECT 
    a.id_expertise_radiologi,c.nomr,
    d.NAMA,
    d.ALAMAT,
    DATE_FORMAT(d.TGLLAHIR, '%d-%m-%Y') AS TGLLAHIR,
    e.nama AS cara_bayar,
    DATE_FORMAT(c.tglreg, '%d-%m-%Y') AS tanggal,
    a.photo,
    f.namadokter AS pengirim,
    h.pd_nickname AS pd_nickname
FROM
    ...
    simrs2012.m_dokter f ON c.kddokter = f.kddokter
        LEFT JOIN
    master_login h ON a.username = h.username
...
<div align="right">
     ...
     <br><br><br>
    <?php echo $DATA_USERNAME['pd_nickname']; ?>
    </div>
</div>
	</div>	
