...
FROM
    ...
    WHERE
        a.id_bill_detail_tarif = $id_bill_detail_tarif and a.userlevelid is null
    ...
    LEFT JOIN simrs2012.vw_diagnosa_eklaim b ON a.icd10 = b.code
    WHERE
        a.id_bill_detail_tarif = $id_bill_detail_tarif and a.userlevelid is null AND a.jenis_diagnosa = 'SEKUNDER') h ON a.id_bill_detail_tarif = h.id_bill_detail_tarif
...
$sqlidentitas1="SELECT anamnesa,suhu_badan AS suhu,
    ...
    heart_rate as nadi,
    respiration_rate as respirasi,
    mata_visus_dasar_od,
    mata_visus_dasar_os,
    mata_visus_koreksi_od,
    mata_visus_koreksi_os,
    mata_tekanan_intraokuler_od,
    mata_tekanan_intraokuler_os from simrs.bill_detail_keterangan where userlevelid <> 31 and  id_bill_detail_tarif=$id_bill_detail_tarif";
...
$sqlidentitas2="SELECT 
    ...
    (SELECT 
        ...
        FROM
            bill_detail_tindakan x
                LEFT OUTER JOIN
            master_tindakan y ON x.id_tindakan = y.id_tindakan
        WHERE
            y.userlevelid = 17
                AND x.id_bill_detail_tarif = a.id_bill_detail_tarif
        GROUP BY x.id_bill_detail_tarif) AS RADIOLOGI,
    (SELECT 
            CONCAT('LABORATORIUM :',
                        GROUP_CONCAT(y.nama_tindakan
        ...
        WHERE
            y.userlevelid = 16
                AND x.id_bill_detail_tarif = a.id_bill_detail_tarif
        GROUP BY x.id_bill_detail_tarif) AS LABORATORIUM,
    (SELECT 
            CONCAT('FISIOTERAPI :',
                        GROUP_CONCAT(y.nama_tindakan
        ...
        WHERE
            y.userlevelid = 31
                AND x.id_bill_detail_tarif = a.id_bill_detail_tarif
        GROUP BY x.id_bill_detail_tarif) AS FISIOTERAPI,
    (SELECT 
            CONCAT('PELAYANAN_DARAH :',
                        GROUP_CONCAT(y.nama_tindakan
        ...
        WHERE
            y.userlevelid = 126
                AND x.id_bill_detail_tarif = a.id_bill_detail_tarif
        GROUP BY x.id_bill_detail_tarif) AS PELAYANAN_DARAH
FROM
    simrs.bill_detail_tarif a
WHERE
    a.id_bill_detail_tarif = $id_bill_detail_tarif";
...
<table width="100%" class="footer" border="0">
  ...
  <tr>
    <td><strong>1. ANAMNESA DAN PEMERIKSAAN</strong><br>
    	...
    	<?php if($DATA_ANAMNESA['suhu_badan'] == '') 
			{ 
				echo '' ;
			} 
			else 
			{
				$str1=", Suhu badan :";
				$str2=" Celcius";
				echo $str1 . " " . $DATA_ANAMNESA['suhu_badan']. " " .$str2;
			}  ?>
    ...
    <?php if($DATA_ANAMNESA['respirasi'] == '') 
      ...
      <!-- ini untuk mata -->
      <?php if($DATA_ANAMNESA['mata_visus_dasar_od'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Visus Dasar OD : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_visus_dasar_od'];
      }  ?>
      <?php if($DATA_ANAMNESA['mata_visus_dasar_os'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Visus Dasar OS : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_visus_dasar_os'];
      }  ?>
      <?php if($DATA_ANAMNESA['mata_visus_koreksi_od'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Visus Koreksi OD : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_visus_koreksi_od'];
      }  ?>
      <?php if($DATA_ANAMNESA['mata_visus_koreksi_os'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Visus Koreksi OS : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_visus_koreksi_os'];
      }  ?>
      <?php if($DATA_ANAMNESA['mata_tekanan_intraokuler_od'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Intraokuler OD : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_tekanan_intraokuler_od'];
      }  ?>
      <?php if($DATA_ANAMNESA['mata_tekanan_intraokuler_os'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        $str1=", Intraokuler OS : ";
        echo $str1 . " " . $DATA_ANAMNESA['mata_tekanan_intraokuler_os'];
      }  ?>
    </td>
  </tr>
  ...
  <tr>
	  ...
	  <?php if($DATA_PENUNJANG['RADIOLOGI'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        echo $DATA_PENUNJANG['RADIOLOGI'].'<br>';
      }  
	  ?>
	  
	  <?php if($DATA_PENUNJANG['LABORATORIUM'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        echo $DATA_PENUNJANG['LABORATORIUM'].'<br>';
      }  
	  ?>
	  
	  <?php if($DATA_PENUNJANG['FISIOTERAPI'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        echo $DATA_PENUNJANG['FISIOTERAPI'].'<br>';
      }  
	  ?>
	  
	  <?php if($DATA_PENUNJANG['PELAYANAN_DARAH'] == '') 
      { 
        echo '' ;
      } 
      else 
      {
        echo $DATA_PENUNJANG['PELAYANAN_DARAH'].'<br>';
      }  
	  ?>
...
$dompdf->render();
$dompdf->stream('48_resume_rajal.pdf',array('Attachment' => 0));
?>