...
session_start();
include('../connect.php');
$id_layanan_sarana_permintaan= $_GET['id_layanan_sarana_permintaan'];
$username = $_GET['username'];

$sqlitem="SELECT 
    b.nama_barang_permintaan, a.jumlah
FROM
    simrs.layanan_sarana_permintaan_detail a
        LEFT JOIN
    master_sarana_permintaan b ON a.id_master_layanan_sarana_permintaan = b.id_master_sarana_permintaan where a.id_layanan_sarana_permintaan=$id_layanan_sarana_permintaan";
...
FROM
    layanan_sarana_permintaan a
        left JOIN
    userlevels b ON a.userlevelid = b.userlevelid
        left JOIN
    userlevels c ON a.userlevelid_tujuan = c.userlevelid where a.id_layanan_sarana_permintaan=$id_layanan_sarana_permintaan";
...
<table width="100%" class="tabel" border="1">
    ...
    <div align="center" class="header"><strong>BUKTI  PERMINTAAN SARANA PRASARANA RUMAH SAKIT RSUD AJIBARANG</strong></div>
  ...
  <tbody>
    ...
    <?php
		...
		while($data=mysql_fetch_assoc($queryitem)){
			echo '<tr>';
	  echo '<td>'.$no.'</td>';
      echo '<td>'.$data['nama_barang_permintaan'].'</td>';
