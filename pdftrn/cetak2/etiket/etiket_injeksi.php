...
FROM
    ...
    master_obat g ON f.id_obat = g.id_obat
        LEFT JOIN
    master_resep h ON a.id_master_resep = h.id_master_resep where a.id_bill_detail_permintaan_obat_master=$id_bill_detail_permintaan_obat_master and a.id_jenis_etiket=4";
      ...
      <?php
				$no=1;
		  		while($data=mysql_fetch_assoc($queryiteminjeksi)){
						echo '<tr>';
						echo '<td colspan="3" class="obat">'.$no.'. '.$data['nama_obat'].' - '.$data['qty'].'</td>';
						echo '</tr>';
			  	$no++;
				}
      ?>
