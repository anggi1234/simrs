...
FROM
	...
	where a.idxdaftar=$idxdaftar and (a.id_profesi=1 or a.id_profesi=13 or a.id_profesi=19)
	group by a.id_profesi, c.kddokter
	order by a.id_profesi asc";
$queryitemdokter = mysqli_query($mysqli, $sqlitemdokter);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemdokter)) {
    echo '<tr>';
    echo '<td></td>';
...
FROM
    ...
    left join master_login c on c.uid=b.uid
    left join master_profesi d on b.id_profesi=d.id_profesi
	where a.idxdaftar=$idxdaftar and (a.id_profesi=12)
	order by a.id_profesi asc";
$queryitemkonsuldokter = mysqli_query($mysqli, $sqlitemkonsultasidokter);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemkonsuldokter)) {
    echo '<tr>';
    echo '<td></td>';
    echo '<td>' . $no . '). ' . $data['pd_nickname'] . '</td>';
    echo '<td align="left">' . $data['nama_profesi'] . '</td>';
...
FROM
    ...
    left join master_login c on c.uid=b.uid
    left join master_profesi d on b.id_profesi=d.id_profesi
	where a.idxdaftar=$idxdaftar and (a.id_profesi=14)
	order by a.id_profesi asc";
$queryitemvct = mysqli_query($mysqli, $sqlitemvct);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemvct)) {
    echo '<tr>';
    echo '<td></td>';
    echo '<td>' . $no . '). ' . $data['pd_nickname'] . '</td>';
    echo '<td align="left">' . $data['nama_profesi'] . '</td>';
...
FROM
    ...
    left join master_login c on c.uid=b.uid
    left join master_profesi d on b.id_profesi=d.id_profesi
	where a.idxdaftar=$idxdaftar and (a.id_profesi=3)
	order by a.id_profesi asc";
$queryitemvisitfarmasi = mysqli_query($mysqli, $sqlitemvisitfarmasi);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemvisitfarmasi)) {
    echo '<tr>';
    echo '<td></td>';
    echo '<td>' . $no . '). ' . $data['pd_nickname'] . '</td>';
    echo '<td align="left">' . $data['nama_profesi'] . '</td>';
...
FROM
    ...
    left join master_login c on c.uid=b.uid
    left join master_profesi d on b.id_profesi=d.id_profesi
	where a.idxdaftar=$idxdaftar and (a.id_profesi=4)
	order by a.id_profesi asc";
$queryitemkonsulgizi = mysqli_query($mysqli, $sqlitemkonsulgizi);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemkonsulgizi)) {
    echo '<tr>';
    echo '<td></td>';
    echo '<td>' . $no . '). ' . $data['pd_nickname'] . '</td>';
    echo '<td align="left">' . $data['nama_profesi'] . '</td>';
...
FROM
    data_tindakan_lain a
        LEFT JOIN
    master_tindakan_lain d ON a.id_tindakan_lain = d.id_master_tindakan_lain
WHERE
    a.idxdaftar = $idxdaftar and a.id_jenis_tindakan=21";
$queryitemrawatintensive = mysqli_query($mysqli, $sqlitemrawatintensive);

$no = 1;
while ($data = mysqli_fetch_assoc($queryitemrawatintensive)) {
    echo '<tr>';
    echo '<td></td>';
    echo '<td>' . $no . '). ' . $data['nama_tindakan'] . '</td>';
...
$dompdf->render();
$dompdf->stream('notaranap.pdf', ['Attachment' => 0]);
?>