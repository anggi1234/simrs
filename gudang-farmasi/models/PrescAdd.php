...
class PrescAdd extends Presc
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("DOSE_PRESC") ?? Route("DOSE_PRESC")) !== null) {
                $this->DOSE_PRESC->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PrescList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PrescList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PrescView") {
    ...
    {
        global $Language, $Security;
        if ($this->DOSE_PRESC->CurrentValue != "") { // Check field with unique index
            $filter = "([DOSE_PRESC] = " . AdjustSql($this->DOSE_PRESC->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->DOSE_PRESC->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->DOSE_PRESC->CurrentValue, $idxErrMsg);
        ...
        // DOSE_PRESC
        $this->DOSE_PRESC->setDbValueDef($rsnew, $this->DOSE_PRESC->CurrentValue, 0, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['DOSE_PRESC']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PrescList"), "", $this->TableVar, true);
