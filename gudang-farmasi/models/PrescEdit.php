...
class PrescEdit extends Presc
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("DOSE_PRESC") ?? Key(0) ?? Route(2)) !== null) {
                $this->DOSE_PRESC->setQueryStringValue($keyValue);
                $this->DOSE_PRESC->setOldValue($this->DOSE_PRESC->QueryStringValue);
            } elseif (Post("DOSE_PRESC") !== null) {
                $this->DOSE_PRESC->setFormValue(Post("DOSE_PRESC"));
                $this->DOSE_PRESC->setOldValue($this->DOSE_PRESC->FormValue);
            } else {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // DOSE_PRESC
            $this->DOSE_PRESC->setDbValueDef($rsnew, $this->DOSE_PRESC->CurrentValue, 0, $this->DOSE_PRESC->ReadOnly);
