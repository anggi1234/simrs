 ...
 * Page class
 */
class PoVerificationEdit extends PoVerification
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'PO_VERIFICATION';

    // Page object name
    public $PageObjName = "PoVerificationEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (PO_VERIFICATION)
        if (!isset($GLOBALS["PO_VERIFICATION"]) || get_class($GLOBALS["PO_VERIFICATION"]) == PROJECT_NAMESPACE . "PO_VERIFICATION") {
            $GLOBALS["PO_VERIFICATION"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'PO_VERIFICATION');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("PO_VERIFICATION"));
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        ...
        if (is_array($ar)) {
            ...
            $key .= @$ar['ISVALID'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['VERIFIED_DATE'];
        }
        return $key;
    }
    ...
    public function run()
    {
        ...
        $this->VERIFICATION_DESC->setVisibility();
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("PO") ?? Key(0) ?? Route(2)) !== null) {
                $this->PO->setQueryStringValue($keyValue);
                $this->PO->setOldValue($this->PO->QueryStringValue);
            } elseif (Post("PO") !== null) {
                $this->PO->setFormValue(Post("PO"));
                $this->PO->setOldValue($this->PO->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("ISVALID") ?? Key(1) ?? Route(3)) !== null) {
                $this->ISVALID->setQueryStringValue($keyValue);
                $this->ISVALID->setOldValue($this->ISVALID->QueryStringValue);
            } elseif (Post("ISVALID") !== null) {
                $this->ISVALID->setFormValue(Post("ISVALID"));
                $this->ISVALID->setOldValue($this->ISVALID->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("VERIFIED_DATE") ?? Key(2) ?? Route(4)) !== null) {
                $this->VERIFIED_DATE->setQueryStringValue($keyValue);
                $this->VERIFIED_DATE->setOldValue($this->VERIFIED_DATE->QueryStringValue);
            } elseif (Post("VERIFIED_DATE") !== null) {
                $this->VERIFIED_DATE->setFormValue(Post("VERIFIED_DATE"));
                $this->VERIFIED_DATE->setOldValue($this->VERIFIED_DATE->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("ISVALID") ?? Route("ISVALID")) !== null) {
                    $this->ISVALID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->ISVALID->CurrentValue = null;
                }
                if (($keyValue = Get("VERIFIED_DATE") ?? Route("VERIFIED_DATE")) !== null) {
                    $this->VERIFIED_DATE->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("PoVerificationList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PoVerificationList") {
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_ISVALID")) {
            $this->ISVALID->setOldValue($CurrentForm->getValue("o_ISVALID"));
        }

        // Check field name 'VERIFIED_DATE' first before field var 'x_VERIFIED_DATE'
        $val = $CurrentForm->hasValue("VERIFIED_DATE") ? $CurrentForm->getValue("VERIFIED_DATE") : $CurrentForm->getValue("x_VERIFIED_DATE");
        if (!$this->VERIFIED_DATE->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->VERIFIED_DATE->Visible = false; // Disable update for API request
            } else {
                $this->VERIFIED_DATE->setFormValue($val);
            }
            $this->VERIFIED_DATE->CurrentValue = UnFormatDateTime($this->VERIFIED_DATE->CurrentValue, 0);
        }
        if ($CurrentForm->hasValue("o_VERIFIED_DATE")) {
            $this->VERIFIED_DATE->setOldValue($CurrentForm->getValue("o_VERIFIED_DATE"));
        }
        ...
        if (!$this->VERIFIED_BY->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->VERIFIED_BY->setFormValue($val);
            }
        }

        // Check field name 'VERIFICATION_DESC' first before field var 'x_VERIFICATION_DESC'
        $val = $CurrentForm->hasValue("VERIFICATION_DESC") ? $CurrentForm->getValue("VERIFICATION_DESC") : $CurrentForm->getValue("x_VERIFICATION_DESC");
        if (!$this->VERIFICATION_DESC->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->VERIFICATION_DESC->Visible = false; // Disable update for API request
            } else {
                $this->VERIFICATION_DESC->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->VERIFICATION_DESC->CurrentValue = $this->VERIFICATION_DESC->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->VERIFIED_DATE->EditValue = HtmlEncode(FormatDateTime($this->VERIFIED_DATE->CurrentValue, 8));
            ...
            $this->VERIFIED_BY->EditValue = HtmlEncode($this->VERIFIED_BY->CurrentValue);
            ...
            $this->VERIFICATION_DESC->EditValue = HtmlEncode($this->VERIFICATION_DESC->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->VERIFIED_DATE->Required) {
            if (!$this->VERIFIED_DATE->IsDetailKey && EmptyValue($this->VERIFIED_DATE->FormValue)) {
                $this->VERIFIED_DATE->addErrorMessage(str_replace("%s", $this->VERIFIED_DATE->caption(), $this->VERIFIED_DATE->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->VERIFIED_DATE->FormValue)) {
            $this->VERIFIED_DATE->addErrorMessage($this->VERIFIED_DATE->getErrorMessage(false));
        }
        if ($this->VERIFIED_BY->Required) {
        ...
        if ($this->VERIFICATION_DESC->Required) {
            if (!$this->VERIFICATION_DESC->IsDetailKey && EmptyValue($this->VERIFICATION_DESC->FormValue)) {
                $this->VERIFICATION_DESC->addErrorMessage(str_replace("%s", $this->VERIFICATION_DESC->caption(), $this->VERIFICATION_DESC->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // ISVALID
            $this->ISVALID->setDbValueDef($rsnew, $this->ISVALID->CurrentValue, "", $this->ISVALID->ReadOnly);

            // VERIFIED_DATE
            $this->VERIFIED_DATE->setDbValueDef($rsnew, UnFormatDateTime($this->VERIFIED_DATE->CurrentValue, 0), CurrentDate(), $this->VERIFIED_DATE->ReadOnly);

            // VERIFIED_BY
            $this->VERIFIED_BY->setDbValueDef($rsnew, $this->VERIFIED_BY->CurrentValue, null, $this->VERIFIED_BY->ReadOnly);

            // VERIFICATION_DESC
            $this->VERIFICATION_DESC->setDbValueDef($rsnew, $this->VERIFICATION_DESC->CurrentValue, null, $this->VERIFICATION_DESC->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PoVerificationList"), "", $this->TableVar, true);
        $pageId = "edit";
