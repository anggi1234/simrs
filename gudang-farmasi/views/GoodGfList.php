...
<input type="hidden" data-table="GOOD_GF" data-field="x_ORG_ID" data-hidden="1" name="x<?= $Page->RowIndex ?>_ORG_ID" id="x<?= $Page->RowIndex ?>_ORG_ID" value="<?= HtmlEncode($Page->ORG_ID->CurrentValue) ?>">
</span>
<?php } ?>
<?php if ($Page->RowType == ROWTYPE_VIEW) { // View record ?>
...
<?php if ($Page->RowType == ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?= $Page->RowCount ?>_GOOD_GF_PRINTQ" class="form-group">
<input type="<?= $Page->PRINTQ->getInputTextType() ?>" data-table="GOOD_GF" data-field="x_PRINTQ" name="x<?= $Page->RowIndex ?>_PRINTQ" id="x<?= $Page->RowIndex ?>_PRINTQ" size="30" placeholder="<?= HtmlEncode($Page->PRINTQ->getPlaceHolder()) ?>" value="<?= $Page->PRINTQ->EditValue ?>"<?= $Page->PRINTQ->editAttributes() ?>>
