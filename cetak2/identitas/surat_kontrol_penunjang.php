...
FROM
    ...
    simrs2012.m_dokter g ON b.kddokter = g.kddokter
        LEFT JOIN
    simrs2012.vw_diagnosa_eklaim h ON d.icd10 = h.code
WHERE a.id_bill_detail_tarif = $id_bill_detail_tarif and a.userlevelid=$userlevelid";
...
$sqlmedis="SELECT 
    c.pd_nickname
FROM
    v_bill_detail_tindakan_penunjang a
        LEFT JOIN
    l_dokter_standby b ON a.l_standby_dokter = b.id_dokter_standby
    left join master_login c on b.uid=c.uid
WHERE
    a.id_bill_detail_tarif = $id_bill_detail_tarif";
 $querymedis = mysql_query($sqlmedis);
 $DATA_MEDIS = mysql_fetch_array($querymedis);
...
<table width="100%" class="tabel">
	...
	<tr>
	  <td align="center">&nbsp;</td>
	  <td align="center">&nbsp;</td>
	  <td align="center"><?php echo $DATA_MEDIS['pd_nickname']; ?></td>
  </tr>
</table>
...
$dompdf->render();
$dompdf->stream('surat_kontrol.pdf',array('Attachment' => 0));
?>