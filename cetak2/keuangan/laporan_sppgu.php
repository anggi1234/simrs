...
$sqldpa="SELECT kode_dpa,tanggal_dpa,nilai_spd FROM simrs.keu_dpa;";
 $querydpa = mysql_query($sqldpa);
	   $querydpa1 = mysql_query($sqldpa);
	   $DATA_DPA = mysql_fetch_array($querydpa1);
...
$queryviewspp = mysql_query($sqlviewspp);  
	  
	  $no=1;
    while($data=mysql_fetch_assoc($queryviewspp)){
					echo '<tr>';
					echo '<td>'.$no.'.</td>';
...
$dompdf->render();
$dompdf->stream('Laporan_SPPGU.pdf',array('Attachment' => 0));
?>