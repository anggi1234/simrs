...
WHERE
    id_jabatan = 4";
 $queryttd = mysql_query($sqlttd);
 $DATA_TTD = mysql_fetch_array($queryttd);
...
$sql="SELECT 
    ...
    a.jumlah_pajak
FROM
    simrs.keu_spj_gu_detail a
        LEFT JOIN
    simrs.keu_spj_gu b ON a.id_spj_gu = b.id_spj_gu
        LEFT JOIN
    simrs.keu_bukti_pengeluaran_gu c ON a.id_bukti_pengeluaran = c.id_bukti_pengeluaran_gu
    where a.id_spj_gu=$id_spj_gu"; 
$query = mysql_query($sql);

$sqltotal="SELECT sum(jumlah) as grandtotal, sum(jumlah_pajak) AS grandtotalpajak from simrs.keu_spj_gu_detail where id_spj_gu=$id_spj_gu";
...
$dompdf->render();
$dompdf->stream('Laporan_SPJ_GU.pdf',array('Attachment' => 0));
?>