...
// print_r($tanggal);
// die();
// $sqlitem="SELECT b.id_bill_detail_tarif, a.ok as OK, c.NAMA, a.nomr, e.NAMA as penjamin, c.TGLLAHIR as tgllahir, c.JENISKELAMIN AS jk, a.kelas, d.NAMADOKTER as DOKTER, f.userlevelname as ruang, SUBSTR(a.jam_mulai_op, 1, 10) as TANGGAL_OP, SUBSTR(a.jam_mulai_op, 11, 14) as JAM_OP,b.op_lensa as lensa,(SELECT 
//             GROUP_CONCAT(IFNULL(z.keterangan, '')
//                     SEPARATOR ', ') AS diagnosa
//         FROM
//             simrs.bill_detail_penyakit z
//             left join simrs.bill_detail_tarif y on z.id_bill_detail_tarif=y.id_bill_detail_tarif
//         WHERE
//             z.idxdaftar = b.idxdaftar and y.userlevelid=b.userlevelid_from
//         GROUP BY z.idxdaftar) AS diagnosa,b.op_rencana_tindakan,
//     b.op_ja,
// CASE
//             WHEN b.op_terlaksana = '0' THEN 'Belum OP'
//             WHEN b.op_terlaksana = '1' THEN 'Sudah OP'
//             ELSE 'Batal'
//        END as status_op
// from simrs.bill_detail_tarif_ibs a
// left join simrs.bill_detail_tarif b on a.id_bill_detail_tarif = b.id_bill_detail_tarif
// left join m_pasien c on c.NOMR = a.nomr
// left join m_dokter d on d.kddokter = a.kddokter
// left join m_carabayar e on e.KODE = a.kdcarabayar
// left join simrs.userlevels f on f.userlevelid = b.userlevelid_from
// left join simrs.master_ruang_op g on g.nama_ruang = a.ok
// where b.id_bill_detail_tarif is not null and (SUBSTR(a.jam_mulai_op, 1, 10) = '".$tanggal."')
// order by g.id asc, a.jam_mulai_op asc";
