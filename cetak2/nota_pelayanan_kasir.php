...
<table width="100%" border="1" class="tabel">
  ...
  <?php
	  $no=a;
	$entridokter = mysql_query("SELECT 
			*
		FROM
	...
	if(mysql_num_rows($entridokter)==0){
	$sqlitemdokter = mysql_query("SELECT 
			...
			FROM
				simrs.bill_detail_tarif a
					LEFT JOIN
				simrs.m_dokter b ON a.kddokter = b.kddokter
					AND a.userlevelid = b.userlevelid
					LEFT JOIN
				simrs.master_login c ON b.kddokter = c.kddokter
					LEFT JOIN
				simrs.master_profesi d ON c.id_profesi = d.id_profesi
			WHERE
				a.id_bill_detail_tarif = $id_bill_detail_tarif and a.kdcarabayar=1");						
	}else{
		$sqlitemdokter = mysql_query("SELECT 
...
FROM
    ...
    left join master_profesi d on b.id_profesi=d.id_profesi where a.id_bill_detail_tarif=$id_bill_detail_tarif order by a.id_profesi asc");
      	...
      	while($data=mysql_fetch_array($sqlitemdokter)){
				echo '<tr>';
				echo '<td></td>';
